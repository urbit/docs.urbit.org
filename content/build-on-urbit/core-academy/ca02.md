---
description: >-
  Core Academy lesson on Urbit's core architecture covering subject-oriented programming, limb resolution mechanics, nested core structures in hoon.hoon, zuse.hoon, and lull.hoon, and the Hoon standard library organization.
---
...
- [`++on`](/hoon/reference/zuse/2m) provides the services for `++mop` ordered maps.
...
- [`++enjs:format`](/hoon/reference/zuse/2d_1-5) supports noun-to-JSON conversions.
...
- [`++json:html`](/hoon/reference/zuse/2e_2-3) tools are reparsers (see `++dejs:format` for the reparsers).
