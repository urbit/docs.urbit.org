...
[`;<` micgal](../../hoon/reference/rune/mic.md#micgal) serves to permit a sequence of computations in which each one depends on the output of the previous one.
...
- `serving` has to do with the [HTTP API](../../urbit-os/base/threads/guides/http-api.md) for threads, allowing you to use the Urbit ship like a function-as-a-service server.
...
- [“Khan:  API Reference”](../../urbit-os/kernel/khan/reference/tasks.md)
...
> The process on the host OS must therefore strip the first 5 bytes, [`++cue`](../../hoon/reference/stdlib/2p.md#cue) the jamfile, check the mark and (most likely) convert the noun into a native data structure.
...
- [`%a` - Read ports](../../urbit-os/kernel/lick/reference/scry.md#a---read-ports)
- [`%d` - Port owner](../../urbit-os/kernel/lick/reference/scry.md#d---port-owner)
- [`%u` - Port existance](../../urbit-os/kernel/lick/reference/scry.md#u---port-existance)
...
- [“`conn.c` Usage Guide”](https://github.com/urbit/build-on-urbit/tools/wiki/conn.c-Usage-Guide)
- [Click](https://github.com/urbit/build-on-urbit/tools/tree/master/pkg/click)
