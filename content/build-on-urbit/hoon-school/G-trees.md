...
If you run into `^$`, don't go look for a `^$` ketbuc rune: it's matching the outer `$` buc arm. `^$` is one way of setting up a `%=` [centis](../../hoon/reference/rune/cen.md#centis) loop/recursion of multiple cores with a `|-` [barhep](../../hoon/reference/rune/bar.md#barhep) trap nested inside of a `|=` [bartis](../../hoon/reference/rune/bar.md#bartis) gate, for instance.
...
where `=|` [tisbar](../../hoon/reference/rune/tis.md#tisbar) means to add its sample to the current subject with the given face.

Similarly, `|-` [barhep](../../hoon/reference/rune/bar.md#barhep) produces a core with one arm `$`. How could you write that in terms of `|%` and `++`?
...
Extend the above generator so that it accepts a cell of type and value (a `$vase` as produced by the `!>` [zapgar](../../hoon/reference/rune/zap.md#zapgar) rune). Use the type to determine which number base the digit string should be constructed from; e.g. `+num2dig !>(0xdead.beef)` should yield `~['d' 'e' 'a' 'd' 'b' 'e' 'e' 'f']`.
...
The [`+roll`](../../hoon/reference/stdlib/2b.md#roll) function takes a list and a gate, and accumulates a value of the list items using that gate. For example, if you want to add or multiply all the items in a list of atoms, you would use roll:
