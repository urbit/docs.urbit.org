...
In this lesson we'll look at scrying agents, as well as how agents handle such scries. If you're not at all familiar with performing scries in general, have a read through the [Scry Guide](urbit-docs/system/kernel/arvo/guides/scry), as well as the [dotket rune documentation](urbit-docs/language/hoon/reference/rune/dot#-dotket).
...
Gall itself defines some special vane-level endpoints [as described in its scry reference](urbit-docs/system/kernel/gall/reference/scry), but most scries to Gall are routed to particular agents and handled by them instead. Agent scries are what we'll focus on here. 

Scries are performed with the [dotket](urbit-docs/language/hoon/reference/rune/dot#-dotket) (`.^`) rune. Here's a summary of their format:
...
Typically, you'd handle the `path` similarly to `on-watch`, as we discussed in the lesson on subscriptions. You'd use something like a [wutlus](urbit-docs/language/hoon/reference/rune/wut#-wutlus) `?+` expression to test the value of the `path`, defining your scry endpoints like so:
...
- Have a read through the [Scry Guide](urbit-docs/system/kernel/arvo/guides/scry).
- Have a look at Gall's [scry reference](urbit-docs/system/kernel/gall/reference/scry).
- Have a read through the [dotket rune documentation](urbit-docs/language/hoon/reference/rune/dot#-dotket).
- Run through the [Example](#example) yourself if you've not done so already.
- Try adding another scry endpoint to the `peeker.hoon` agent, which uses a [`wyt:by`](urbit-docs/language/hoon/reference/stdlib/2i#wytby) map function to produce the number of items in the `data` map.
