...
A Gall agent is a [door](../../glossary/door.md) with exactly ten [arms](../../glossary/arm.md). Each arm is responsible for handling certain kinds of events that Gall feeds in to the agent. A door is just a [core](../../glossary/core.md) with a sample - it's made with the [barcab](../../hoon/reference/rune/bar.md#_-barcab) "BROKEN_ANCHOR" rune (`|_`) instead of the usual [barcen](../../hoon/reference/rune/bar.md#barcen) rune (`|%`).
...
- Have a look at the [`$bowl` entry in the Gall data types documentation](../../system/kernel/gall/reference/data-types.md#bowl "BROKEN_LINK") if you've not done so already.
