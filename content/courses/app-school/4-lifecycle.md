...
A `$vase` is just a cell of \[type-of-the-noun the-noun]. Most data an agent sends or receives will be encapsulated in a vase. A `$vase` is made with the [zapgar](../../hoon/reference/rune/zap.md#zapgar) (`!>`) rune like `!>(some-data)`, and unpacked with the [zapgal](../../hoon/reference/rune/zap.md#zapgal) (`!<`) rune like `!<(type-to-extract vase)`. Have a read through the [`$vase` section of the type reference for details](types.md#vase).
...
In addition to each of those individual state versions, you'd also define a structure called `$versioned-state`, which just contains a union of all the possible states. This way, the `$vase` `+on-load` receives can be unpacked to a `$versioned-state` type, and then a [wuthep](../../hoon/reference/rune/wut.md#wuthep) (`?-`) expression can switch on the head (`%0`, `%1`, `%2`, etc) and process each one appropriately.
...
The state is packed in a `$vase` with the [zapgar](../../hoon/reference/rune/zap.md#zapgar) (`!>`) rune, like `!>(state)`.
...
The `$vase` would be unpacked with a [zapgal](../../hoon/reference/rune/zap.md#zapgal) (`!<`) rune, and then typically you'd test its version with a [wuthep](../../hoon/reference/rune/wut.md#wuthep) (`?-`) expression.
...
We've just bunted the `$state-0` type, which will produce `[%0 val=0]`, pinning it to the head of the subject. Then, we've use [tistar](../../hoon/reference/rune/tis.md#tistar) (`=*`) to give it a name of `.state`.
