...
An agent can send pokes to other agents by producing [`%poke` `card`s](5-cards#pokes). Any agent arm apart from `on-peek` and `on-save` can produce such `card`s. The arms would typically produce the `(quip card _this)` like so:
...
It's basically the same as a [`gift`](5-cards#give), but incoming instead of outgoing.
