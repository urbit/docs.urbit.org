...
- [Nock Definition](urbit-docs/reference/nock/definition)
- [Nock Explanation](urbit-docs/reference/nock/explanation)
...
The following paragraphs summarize [the reference explanation](urbit-docs/reference/nock/explanation#1-constant) for these Nock rules.  Two runes and one gate will be particularly useful as we analyze Nock:

- We will evaluate Nock statements in Hoon using [`.*` dottar](urbit-docs/reference/hoon/rune/dot#-dottar) or Nock Two.
- We can produce the Nock equivalent of a Hoon statement using [`!=` zaptis](urbit-docs/reference/hoon/rune/zap#-zaptis), which takes a single `hoon` for its sample.
- We can run a virtualized Nock (with crash data) in [`++mock`](urbit-docs/reference/hoon/stdlib/4n#mock).  An optional hint handler can be supplied but we will typically ignore it.
...
The funkiness of the equation is because we have to make sure that the result is `0` or `1`; we must crash on another value or on a cell.  The [docs](urbit-docs/reference/nock/explanation#6-if-then-else) demonstrate a simpler version of Nock Six.
...
There's another way around this, which will also be illuminating:  build it from the [ground up](urbit-docs/reference/nock/example).
...
- [“Writing Jets”](urbit-docs/reference/runtime/jetting)
...
- [“Concepts:  Scry”](urbit-docs/reference/arvo/concepts/scry)
- [“Additional Guides:  Remote Scry”](urbit-docs/guides/additional/remote-scry)
...
- [`u3n`:  Nock Execution](urbit-docs/reference/runtime/api#u3n-nock-execution)
...
- [“API Overview by Prefix:  `u3j` jets”](urbit-docs/reference/runtime/api#u3j-jets)
...
- [Vere, “Land of Nouns:  `u3` the road model”](urbit-docs/reference/runtime/nouns#u3-the-road-model)
