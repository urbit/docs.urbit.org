...
The [`++slap`/`++slop` vase algebra](urbit-docs/guides/additional/vases#slap'n'slop-vase-algebra) provides a framework for working through Hoon expressions at an abstract level.

#### `++slap`:  Envase Hoon

[`++slap`](urbit-docs/reference/hoon/stdlib/5c#slap) runs a `hoon` against a `vase` and produces a `vase` of the result.
...
[`++slop`](urbit-docs/reference/hoon/stdlib/5c#slop) combines a `cell` of `vase`s into a `vase` of a `cell`.
...
[`++slam`](urbit-docs/reference/hoon/stdlib/5c#slam) accepts a gate as a `vase` and appropriate arguments.
...
[`++slot`](urbit-docs/reference/hoon/stdlib/5c#slot) can be used to decompose vases.
...
- [Hoon, “Vases”](urbit-docs/guides/additional/vases)
...
[`++ride`](urbit-docs/reference/hoon/stdlib/5d#ride) accepts a pair of `+$type` and an atom (really a `cord`) of text containing Hoon code.
...
As another example, [`++make`](urbit-docs/reference/hoon/stdlib/5d#make) shows the process in capsule form for converting a `cord` of Hoon code into executable `+$nock` Nock code.  `++make` doesn't explicitly require or deal with `+$type` like `++ride` does.
...
To take a `cord` containing Hoon code and transform it to an AST, use [`+ream`](urbit-docs/reference/hoon/stdlib/5d#ream):
...
- [Hoon, “Constants (Atoms and Strings)”](urbit-docs/reference/hoon/rune/constants)

- Examine `++open:ap` in `/sys/hoon.hoon` to see how desugaring proceeds.

### AST `+$hoon` → Nock `+$nock`

All Hoon code is ultimately run by `++mint:ut`, although there can be many paths there.  `++mint:ut` parses from a Hoon AST into a pair of the type and the Nock.  What is [`++ut`](urbit-docs/reference/hoon/stdlib/5c#ut)?  It's the Hoon compiler backend, containing all of the arms necessary to actually process a `+$hoon` into a `+$nock`.
...
`++find` is a wing resolution arm.  It tries Nock Zero first to see if the wing is a leg, then tries Nock Nine if that fails.  It also needs information about the core variance (`%read`, `%rite`, `%both`, `%free`); see [`++slab`](urbit-docs/reference/hoon/stdlib/5c#slab) as well, which also uses `++fond` under the hood.
...
The [`|$` barbuc](urbit-docs/reference/hoon/rune/bar#-barbuc) rune is used to build a mold builder gate.  (As such, it's fairly abstract.)  Essentially, it is sugar for a certain use of [`|*` bartar](urbit-docs/reference/hoon/rune/bar#-bartar):
...
- Exercise:  Write your own [`++slam`](urbit-docs/reference/hoon/stdlib/5c#slam) using `++slap` and `++slop`.
