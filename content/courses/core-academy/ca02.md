# 3. The Core Stack {#3-the-core-stack}
...
#### Aside:  Constructing Gates {#aside-constructing-gates}
...
##### Insecure Hashing {#insecure-hashing}
...
##### Unsigned Powers {#unsigned-powers}
...
##### Container Logic {#container-logic}
...
##### Various Type Definitions {#various-type-definitions}
...
##### Parsing and Compiler {#parsing-and-compiler}
...
##### Sail and XML Parsing {#sail-and-xml-parsing}
...
- [`++on`](/reference/hoon/zuse/2m) provides the services for `++mop` ordered maps.
...
##### Networking (Ames) {#networking-ames}
...
##### Timekeeping (Behn) {#timekeeping-behn}
...
##### Versioning (Clay) {#versioning-clay}
...
##### HTTP Server (Eyre) {#http-server-eyre}
...
##### Extensions (Gall) {#extensions-gall}
...
##### HTTP Client (Iris) {#http-client-iris}
...
##### Formatting Text and JSON Reparsing {#formatting-text-and-json-reparsing}
...
- [`++enjs:format`](/reference/hoon/zuse/2d_1-5) supports noun-to-JSON conversions.
...
##### Web Text (HTML &c.) {#web-text-html-c}
...
- [`++json:html`](/reference/hoon/zuse/2e_2-3) tools are reparsers (see `++dejs:format` for the reparsers).
...
##### Millisecond Timing {#millisecond-timing}
...
#### The Compilation Subject {#the-compilation-subject}
