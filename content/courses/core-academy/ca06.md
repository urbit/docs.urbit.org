...
- [“Land of Nouns”, section “`u3`:  the road model”](urbit-docs/reference/runtime/nouns#u3-the-road-model)
- [“API overview by prefix”, section “`u3a`:  allocation functions”](urbit-docs/reference/runtime/api#u3a-allocation-functions)
...
> Replay is how [Vere](urbit-docs/reference/glossary/vere) computes the state of a ship's [Arvo](urbit-docs/reference/glossary/arvo) instance from the [event log](urbit-docs/reference/glossary/eventlog) after a ship reboots. In order to avoid replaying the entire event log, Replay takes a snapshot of the current state of the ship approximately once every ten minutes. Then when a ship reboots, Replay loads the most recent snapshot and replays events from the event log up to the most recent event.
...
> Khan is the "control plane" and thread-runner vane. Its main purpose is to allow external applications to run [threads](urbit-docs/reference/arvo/threads/overview) via a Unix Socket and receive the result.
...
- [`%khan` Overview](urbit-docs/reference/arvo/khan/khan)
...
- [`%lick` Overview](urbit-docs/reference/arvo/lick/lick)
