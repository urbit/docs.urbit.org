...
[`;<` micgal](urbit-docs/language/hoon/reference/rune/mic#-micgal) serves to permit a sequence of computations in which each one depends on the output of the previous one.
...
- `serving` has to do with the [HTTP API](urbit-docs/userspace/threads/guides/http-api) for threads, allowing you to use the Urbit ship like a function-as-a-service server.
...
- [“Khan”](urbit-docs/system/kernel/khan)
...
- [“Khan:  API Reference”](urbit-docs/system/kernel/khan/reference/tasks)
...
> The process on the host OS must therefore strip the first 5 bytes, [`++cue`](urbit-docs/language/hoon/reference/stdlib/2p#cue) the jamfile, check the mark and (most likely) convert the noun into a native data structure.
...
- [`%a` - Read ports](urbit-docs/system/kernel/lick/reference/scry#a---read-ports)
- [`%d` - Port owner](urbit-docs/system/kernel/lick/reference/scry#d---port-owner)
- [`%u` - Port existance](urbit-docs/system/kernel/lick/reference/scry#u---port-existance)
