...
> These values are utilized by [Ames](../../system/kernel/ames) "BROKEN_LINK" and [Jael](../../urbit-os/kernel/jael) to ensure that communication between ships is always done with the most recent set of networking keys, and that networking state is appropriately reset when a breach has occurred.
...
- [“Comet self-attestation”](../../system/kernel/ames/guides/cryptography.md#comets) "BROKEN_LINK"

### Ecliptic {#ecliptic}

There are two contract addresses for Azimuth:

- [`Azimuth.eth`](../../urbit-id/reference/azimuth-eth.md) is the contract data store containing the actual state of Azimuth on Ethereum.
- [`Ecliptic.eth`](../../urbit-id/reference/ecliptic.md) contains the operational logic (transfers, unlocking, etc.). It is separate from `Azimuth.eth` so that the contracts can be upgraded without losing chain state.
...
- [“Identity:  Smart contracts”](../../system/identity#smart-contracts) "BROKEN_LINK"
...
- [“Azimuth Data Flow:  `%eth-watcher`”](../../urbit-id/concepts/flow.md#eth-watcher)
- [“Azimuth Data Flow:   `naive.hoon`](../../urbit-id/concepts/flow.md#naive)
