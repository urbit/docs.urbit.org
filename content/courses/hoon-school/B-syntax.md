...
An [**expression**](https://en.wikipedia.org/wiki/Expression_%28computer_science%29) is a combination of characters that a language interprets and evaluates to produce a value.  All Hoon programs are built of expressions, rather like mathematical equations.  Hoon expressions are built along a backbone of [runes](urbit-docs/glossary/rune), which are two-character symbols that act like keywords in other programming languages to define the syntax, or grammar, of the expression.
...
At its root, Urbit is completely specified by [Nock](urbit-docs/glossary/nock), sort of a machine language for the Urbit virtual machine layer and event log.  However, Nock code is basically unreadable (and unwriteable) for a human.  [One worked example](urbit-docs/language/nock/examples/decrement) yields, for decrementing a value by one, the Nock formula:
...
The most general data category in Hoon is a [noun](urbit-docs/glossary/noun). This is just about as broad as saying “thing”, so let's be more specific:

> A noun is an atom or a cell.

Progress?  We can say, in plain English, that

- An [atom](urbit-docs/glossary/atom) is a non-negative integer number (0 to +∞), e.g. `42`.
- A [cell](urbit-docs/glossary/cell) is a pair of two nouns, written in square brackets, e.g. `[0 1]`.
...
If an atom is a non-negative number, how do we represent anything else? Hoon provides each atom an [aura](urbit-docs/glossary/aura) , a tag which lets you treat a number as text, time, date, Urbit address, IP address, and much more.
...
The `^-` [kethep](urbit-docs/language/hoon/reference/rune/ket#--kethep) rune is useful for ensuring that everything in the second child matches the type (aura) of the first, e.g.
...
Convert between some of the given auras at the [Dojo](urbit-docs/glossary/dojo) prompt, e.g.:
...
A [cell](urbit-docs/glossary/cell) is a pair of nouns. Cells are traditionally written using square brackets:  `[]`.  For now, just recall the square brackets and that cells are always _pairs_ of values.
...
This is actually a shorthand for a rune as well, `:-` [colhep](urbit-docs/language/hoon/reference/rune/col#--colhep)
...
The backbone of any Hoon expression is a scaffolding of [runes](urbit-docs/glossary/rune) , which are essentially mathematical relationships between daughter components.  If nouns are nouns, then runes are verbs:  they describe how nouns relate.  Runes provide the structural and logical relationship between noun values.

A rune is just a pair of ASCII characters (a digraph).  We usually [pronounce runes](urbit-docs/glossary/aural-ascii) by combining their characters’ names, e.g.: ["kethep"](urbit-docs/language/hoon/reference/rune/ket#--kethep) for `^-`, ["bartis"](urbit-docs/language/hoon/reference/rune/bar#-bartis) for `|=`, and ["barcen"](urbit-docs/language/hoon/reference/rune/bar#-barcen) for `|%`.

For instance, when we called a function earlier (in Hoon parlance, we _slammed a gate_), we needed to provide the `%-` [cenhep](urbit-docs/language/hoon/reference/rune/cen#-cenhep) rune with two bits of information, a function name and the values to associate with it:
...
The [++add](urbit-docs/language/hoon/reference/stdlib/1a#add) function expects precisely two values (or _arguments_), which are provided by [%-](urbit-docs/language/hoon/reference/rune/cen#-cenhep) in the neighboring child expression as a cell.  There's really no limit to the complexity of Hoon expressions:  they can track deep and wide.  They also don't care much about layout, which leaves you a lot of latitude. The only hard-and-fast rule is that there are single spaces (`ace`s) and everything else (`gap`s).
...
- [`++add`](urbit-docs/language/hoon/reference/stdlib/1a#add) (addition)
- [`++sub`](urbit-docs/language/hoon/reference/stdlib/1a#sub) (subtraction, positive results only—what happens if you subtract past zero?)
- [`++mul`](urbit-docs/language/hoon/reference/stdlib/1a#mul) (multiplication)
- [`++div`](urbit-docs/language/hoon/reference/stdlib/1a#div) (integer division, no remainder)
- [`++pow`](urbit-docs/language/hoon/reference/stdlib/2g#pow) (power or exponentiation)
- [`++mod`](urbit-docs/language/hoon/reference/stdlib/1a#mod) (modulus, remainder after integer division)
- [`++dvr`](urbit-docs/language/hoon/reference/stdlib/1a#dvr) (integer division with remainder)
- [`++max`](urbit-docs/language/hoon/reference/stdlib/1a#max) (maximum of two numbers)
- [`++min`](urbit-docs/language/hoon/reference/stdlib/1a#min) (minimum of two numbers)
...
Here is a lightly-edited snippet of Hoon code.  Anything written after a `::` [colcol](urbit-docs/language/hoon/reference/rune/col#-colcol) is a _comment_ and is ignored by the computer.  (Comments are useful for human-language explanations.)
...
Without looking it up first, what does the `==` [tistis](urbit-docs/language/hoon/reference/rune/terminators#-tistis) do for the `:~` [colsig](urbit-docs/language/hoon/reference/rune/col#-colsig) rune?  Hint: some runes can take any number of arguments.

Most runes are used at the beginning of a complex expression, but there are exceptions. For example, the runes `--` [hephep](urbit-docs/language/hoon/reference/rune/terminators#---hephep) and `==` [tistis](urbit-docs/language/hoon/reference/rune/terminators#-tistis) are used at the end of certain expressions.
...
Runes are classified by family (with the exceptions of `--` hephep and `==` tistis). The first of the two symbols indicates the family—e.g., the `^-` kethep rune is in the `^` [ket](urbit-docs/language/hoon/reference/rune/ket) family of runes, and the `|=` bartis and `|%` barcen runes are in the `|` [bar](urbit-docs/language/hoon/reference/rune/bar) family.  The runes of particular family usually have related meanings.  Two simple examples: the runes in the `|` bar family are all used to create cores, and the runes in the `:` [col](urbit-docs/language/hoon/reference/rune/col) family are all used to create cells.
...
Nearly all rune expressions can be written in either form, but there are exceptions.  `|%` [barcen](urbit-docs/language/hoon/reference/rune/bar#-barcen) and `|_` [barcab](urbit-docs/language/hoon/reference/rune/bar#_-barcab) expressions, for example, can only be written in tall form.  (Those are a bit too complicated to fit comfortably on one line anyway.)

### Nesting Runes

Since runes take a fixed number of children, one can visualize how Hoon expressions are built by thinking of each rune being followed by a series of boxes to be filled—one for each of its children.  Let us illustrate this with the `:-` [colhep](urbit-docs/language/hoon/reference/rune/col#--colhep) rune.
...
When we used [++add](urbit-docs/language/hoon/reference/stdlib/1a#add) or [++sub](urbit-docs/language/hoon/reference/stdlib/1a#sub) previously, we wanted an immediate answer.  There's not much more to say than `5 + 1`.  In contrast, pinning a face accepts three daughter expressions:  a name (or face), a value, and the rest of the expression.
...
There are two ways to represent text in Urbit:  cords (`@t` [aura](urbit-docs/glossary/aura) atoms) and [tapes](urbit-docs/glossary/tape) (lists of individual characters).  Both of these are commonly called [“strings”](https://en.wikipedia.org/wiki/String_%28computer_science%29).
...
A [cord](urbit-docs/glossary/cord) (`@t`) represents text as a sequence of characters.  If you know the [ASCII](https://en.wikipedia.org/wiki/ASCII) value for a particular character, you can identify how the text is structured as a number. (This is most easily done using the hexadecimal `@ux` representation due to bit alignment.)
...
- [`++gth`](urbit-docs/language/hoon/reference/stdlib/1a#gth) (greater than `>`)
- [`++lth`](urbit-docs/language/hoon/reference/stdlib/1a#lth) (less than `<`)
- [`++gte`](urbit-docs/language/hoon/reference/stdlib/1a#gte) (greater than or equal to `≥`)
- [`++lte`](urbit-docs/language/hoon/reference/stdlib/1a#lte) (less than or equal to `≤`)
