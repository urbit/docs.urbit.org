...
2. [Molds](urbit-docs/glossary/mold):  structures. Think of cells, lists, and sets.
3. [Marks](urbit-docs/glossary/mark):  file types. Compare to conventional files distinguished by extension and definite internal structure.
...
You have seen and worked with the trivial atoms and cells.  We will leave marks until a later discussion of [Gall](urbit-docs/glossary/gall) [agents](urbit-docs/glossary/agent) or the [Clay](urbit-docs/glossary/clay) filesystem, which use marks to type file data. For now, we focus on [molds](urbit-docs/glossary/mold).
...
The piece of type information that determines how Hoon interprets an atom is called an [aura](urbit-docs/glossary/aura). The set of all atoms is indicated with the symbol `@`.  An aura is indicated with `@` followed by some letters, e.g., `@ud` for unsigned decimal.  Accordingly, the Hoon type system does more than track sets of values.  It also tracks certain other relevant metadata about how those values are to be interpreted.
...
What exactly does the `^-` [kethep](urbit-docs/language/hoon/reference/rune/ket#--kethep) rune do?  It compares the inferred type of some expression with the desired cast type.  If the expression's inferred type _nests_ under the desired type, then the product of the expression is returned.
...
Because `@ud` is the inferred type of `15`, the cast succeeds.  Notice that the `^-` kethep expression never does anything to modify the underlying [noun](urbit-docs/glossary/noun) of the second subexpression.  It's used simply to mandate a type-check on that expression.  This check occurs at compile-time (when the expression is compiled to [Nock](urbit-docs/glossary/nock)).
...
Some of these auras nest under others.  For example, `@u` is for all unsigned auras.  But there are other, more specific auras; `@ub` for unsigned binary numbers, `@ux` for unsigned hexadecimal numbers, etc. (For a more complete list of auras, see [Auras](urbit-docs/language/hoon/reference/auras).)
...
The `[@ @]` cast accepts any expression that evaluates to a cell with exactly two atoms, and crashes with a [nest-fail](urbit-docs/language/hoon/reference/hoon-errors#nest-fail) for any expression that evaluates to something different.  The expression `12` doesn't evaluate to a cell; and while the expression `[[12 13] 14]` does evaluate to a cell, the left-hand side isn't an atom, but is instead another cell.
...
2.  Produce an example value ([bunt](urbit-docs/glossary/bunt)).
...
To _actually_ get the bunt value, use the `^*` [kettar](urbit-docs/language/hoon/reference/rune/ket#-kettar) rune, almost always used in its irregular form `*` tar:
...
One more way to validate against type is to use an example instead of the extracted mold.  This uses the `^+` [ketlus](urbit-docs/language/hoon/reference/rune/ket#-ketlus) rune similarly to how we used `^-` [kethep](urbit-docs/language/hoon/reference/rune/ket#--kethep) previously:
...
We can use more complex structures for molds though, including built-in types like [lists](urbit-docs/glossary/list) and [tapes](urbit-docs/glossary/tape).  (A `tape` represents text.)
...
Besides `?` (which is a Dojo-specific tool), the programmatic way to figure out which mold the Hoon compiler thinks something is to use the `!>` [zapgar](urbit-docs/language/hoon/reference/rune/zap#-zapgar) rune.
...
`$?` [bucwut](urbit-docs/language/hoon/reference/rune/buc#-bucwut) forms a type union. Most commonly these are used with types having different structures, such as an atom and a cell.
