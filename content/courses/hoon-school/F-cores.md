...
The Hoon subject is a noun.  One way to look at this noun is to denote each fragment of is as either a computation or data.  By strictly separating these two kinds of things, we derive the data structure known within Hoon as a [core](urbit-docs/glossary/core).

Cores are the most important data structure in Hoon.  They allow you to solve many coding problems by identifying a pattern and supplying a proper data structure apt to the challenge.  You have already started using cores with `|=` [bartis](urbit-docs/language/hoon/reference/rune/bar#-bartis) [gate](urbit-docs/glossary/gate) construction and use.

This lesson will introduce another [core](urbit-docs/glossary/core) to solve a specific use case, then continue with a general discussion of cores. Getting cores straight will be key to understanding why Hoon has the structure and internal logic it does.
...
Hoon effects the concept of a loop using recursion, return to a particular point in an expression (presumably with some different values).  One way to do this is using the `|-` [barhep](urbit-docs/language/hoon/reference/rune/bar#--barhep) rune, which creates a structure called a [trap](urbit-docs/glossary/trap).  (Think of the “trap” in the bottom of your sink.)  It means a point to which you can return again, perhaps with some key values (like a counter) changed.  Then you can repeat the calculation inside the trap again.  This continues until some single value, some noun, results, thereby handing a value back out of the expression.  (Remember that every Hoon expression results in a value.)
...
It is frequently helpful, when constructing these, to be able to output the values at each step of the process.  Use the `~&` [sigpam](urbit-docs/language/hoon/reference/rune/sig#-sigpam) rune to create output without changing any values:
...
- Let's calculate a [factorial](https://mathworld.wolfram.com/Factorial.html).  The factorial of a number $$n$$ is $$n \times (n-1) \times \ldots \times 2 \times 1$$.  We will introduce a couple of new bits of syntax and a new gate ([++dec](urbit-docs/language/hoon/reference/stdlib/1a#dec)).  Make this into a generator `factorial.hoon`:
    ...
    - We are using the `=` irregular syntax for the `.=` [dottis](urbit-docs/language/hoon/reference/rune/dot#-dottis) rune, which tests for the equality of two expressions.
...
As we write more complicated programs, it is helpful to learn to read the [runes](urbit-docs/glossary/rune) by identifying which daughter expressions attach to which runes, e.g.:
...
We will revert to the irregular form more and more.  If you would like to see exactly how an expression is structured, you can use the `!,` [zapcom](urbit-docs/language/hoon/reference/rune/zap#-zapcom) rune.  `!,` zapcom produces an annotated _abstract syntax tree_ (AST) which labels every value and expands any irregular syntax into the regular runic form.
...
(_There's a lot going on in there._  Focus on the four-letter runic identifiers:  `%sgpm` for `~&` [sigpam](urbit-docs/language/hoon/reference/rune/sig#-sigpam), for instance.)
...
Produce a gate (generator) which accepts a [tape](urbit-docs/glossary/tape) value and returns a `(list @ud)` containing the ASCII value of each character. Use a `|-` [barhep](urbit-docs/language/hoon/reference/rune/bar#--barhep) [trap](urbit-docs/glossary/trap). The previous code simply modified a value by addition.  You can generalize this to other arithmetic processes, like multiplication, but you can also grow a data structure like a [list](urbit-docs/glossary/list).
...
- You can retrieve the _n_^th^ element in a `tape` using the [++snag](urbit-docs/language/hoon/reference/stdlib/2b#snag) gate, e.g. ``(snag 3 `(list @ud)`~[1 2 3 4 5])`` yields `4` (so `++snag` is zero-indexed; it counts from zero).
- You can join an element to a list using the [`++snoc`](urbit-docs/language/hoon/reference/stdlib/2b#snoc) gate, e.g. ``(snoc `(list @ud)`~[1 2 3] 4)`` yields `~[1 2 3 4]`.
...
A [core](urbit-docs/glossary/core) is a cell pairing operations to data. Formally, we'll say a core is a cell `[battery payload]`, where [battery](urbit-docs/glossary/battery) describes the things that can be done (the operations) and [payload](urbit-docs/glossary/payload) describes the data on which those operations rely.  (For many English speakers, the word “battery” evokes a [voltaic pile](https://en.wikipedia.org/wiki/Voltaic_pile) more than a bank of guns, but the artillery metaphor is a better mnemonic for `[battery payload]`.)

**Cores are the most important structural concept for you to grasp in Hoon.**  Everything nontrivial is a core.  Some of the runes you have used already produce cores, like the gate.  That is, a gate marries a `battery` (the operating code) to the `payload` (the input values AND the [subject](urbit-docs/glossary/subject) or operating context).

Urbit adopts an innovative programming paradigm called [subject-oriented programming](urbit-docs/glossary/subject-oriented-programming).  By and large, Hoon (and [Nock](urbit-docs/glossary/nock)) is a functional programming language in that running a piece of code twice will always yield the same result, and because runs cause a program to explicitly compose various subexpressions in a somewhat mathematical way.

Hoon (and Nock) very carefully bounds the known context of any part of the program as the [subject](urbit-docs/glossary/subject).  Basically, the subject is the noun against which any arbitrary Hoon code is evaluated.

For instance, when we first composed generators, we made what are called “naked generators”:  that is, they do not have access to any information outside of the base subject (Arvo, Hoon, and `%zuse`) and their [sample](urbit-docs/glossary/sample) (arguments).  Other [generators](urbit-docs/glossary/generator) (such as `%say` generators, described below) can have more contextual information, including random number generators and optional arguments, passed to them to form part of their subject.

Cores have two kinds of values attached:  [arms](urbit-docs/glossary/arm) and _legs_, both called limbs.  Arms describe known labeled addresses (with `++` luslus or `+$` lusbuc) which carry out computations.  Legs are limbs which store data (with e.g. `=/` tisfas).
...
An [arm](urbit-docs/glossary/arm) is some expression of Hoon encoded as a noun.  (By 'encoded as a noun' we literally mean: 'compiled to a Nock formula'.  But you don't need to know anything about [Nock](urbit-docs/glossary/nock) to understand Hoon.) You virtually never need to treat an arm as raw data, even though technically you can—it's just a noun like any other.  You almost always want to think of an arm simply as a way of running some Hoon code.

Every expression of Hoon is evaluated relative to a subject.  An [arm](urbit-docs/glossary/arm) is a Hoon expression to be evaluated against the [core](urbit-docs/glossary/core) subject (i.e. its parent core is its subject).

#### Arms for Gates

Within a core, we label arms as Hoon expressions (frequently `|=` bartis gates) using the `++` [luslus](urbit-docs/language/hoon/reference/rune/lus#-luslus) digraph.  (`++` isn't formally a rune because it doesn't actually change the structure of a Hoon expression, it simply marks a name for an expression or value. The `--` [hephep](urbit-docs/language/hoon/reference/rune/terminators#---hephep) limiter digraph is used because `|%` [barcen](urbit-docs/language/hoon/reference/rune/bar#-barcen) can have any number of arms attached.  Like `++`, it is not formally a rune.)
...
We can define custom types for a core using `+$` [lusbuc](urbit-docs/language/hoon/reference/rune/lus#-lusbuc) digraphs.  We won't do much with these yet but they will come in handy for custom types later on.
...
When we write generators, we can include helpful tools as arms either before the main code (with `=>` [tisgar](urbit-docs/language/hoon/reference/rune/tis#-tisgar)) or after the main code (with `=<` [tisgal](urbit-docs/language/hoon/reference/rune/tis#-tisgal)):
...
A library (a file in `/lib`) is typically structured as a `|%` [barcen](urbit-docs/language/hoon/reference/rune/bar#-barcen) core.
...
Often a leg of the subject is produced with its value unchanged. But there is a way to produce a modified version of the leg as well. To do so, we use the `%=` [centis](urbit-docs/language/hoon/reference/rune/cen#-centis) rune:
...
This statement means that we recalculate the `$` buc arm of the current subject with the indicated changes.  But what is `$` buc?  `$` buc is the _default arm_ for many core structures, including `|=` [bartis](urbit-docs/language/hoon/reference/rune/bar#-bartis) gate cores and `|-` [barhep](urbit-docs/language/hoon/reference/rune/bar#--barhep) trap cores.
...
1.  The [battery](urbit-docs/glossary/battery) of a gate contains an arm which has the special name `$` buc.  The `$` buc arm contains the instructions for the function in question.
2.  The [payload](urbit-docs/glossary/payload) of a gate consists of a cell of `[sample context]`.
    1.  The [sample](urbit-docs/glossary/sample) is the part of the payload that stores the "argument" (i.e., input value) of the function call.
...
We can write this code in several ways using the `%=` [centis](urbit-docs/language/hoon/reference/rune/cen#-centis) plus `$` buc structure.
...
The (only) [arm](urbit-docs/glossary/arm) of a [gate](urbit-docs/glossary/gate) encodes the instructions for the Hoon function in question.
...
The [sample](urbit-docs/glossary/sample) of a gate is the address reserved for storing the argument(s) to the Hoon function. Although we don't know about addressing yet, you saw above that `+2` referred to the battery.  The sample is always at the head of the gate's tail, `+6`.  (We'll look at addressing in more depth in [the next module](urbit-docs/courses/hoon-school/G-trees).)
...
We see now that the sample of `inc` is the value `0`, and has `a` as a [face](urbit-docs/glossary/face).  This is a placeholder value for the function argument.  If you evaluate the `$` buc arm of `inc` without passing it an argument the placeholder value is used for the computation, and the return value will thus be `0+1`:
...
The placeholder value, as you saw in the previous module, is sometimes called the [bunt](urbit-docs/glossary/bunt) value.  The bunt value is determined by the input type; for `@` atoms the bunt value is typically `0`.
...
This is the default Dojo subject from before we put `inc` into the subject. The `|=` [bartis](urbit-docs/language/hoon/reference/rune/bar#-bartis) expression defines the context as whatever the subject is.  This guarantees that the context has all the information it needs to have for the `$` buc arm to work correctly.
...
Notice that neither the arm nor the context is modified before the arm is evaluated.  That means that the only part of the gate that changes before the arm evaluation is the [sample](urbit-docs/glossary/sample).  Hence, we may understand each gate as defining a function whose argument is the sample.  If you call a gate with the same sample, you'll get the same value returned to you every time.
...
In the following code, the `|-` [barhep](urbit-docs/language/hoon/reference/rune/bar#--barhep) [trap](urbit-docs/glossary/trap) serves as the point of recursion, and the return to that point (with changes) is indicated by the `%=` centis.  All this code does is count to the given number, then return that number.
...
We are using the `+` irregular syntax for the `.+` [dotlus](urbit-docs/language/hoon/reference/rune/dot#-dotlus) rune, which increments a value (adds one).
...
You need to make sure when you compose a [trap](urbit-docs/glossary/trap) that it has a base case which returns a noun. The following trap results in an infinite loop:
...
- Compose a Fibonacci sequence program which produces a [list](urbit-docs/glossary/list) of the appropriate values.
    ...
    We can use _two_ recursion points for `%=` [centis](urbit-docs/language/hoon/reference/rune/cen#-centis).  The first calculate $$F$$ for $$n-1$$; the second calculate $$F$$ for $$n-2$$.  These are then added together.  If we diagram what's happening, we can see that each additional number costs as much as the previous numbers:
    ...
    An improved version stores each value in the sequence as an element in a list so that it can be used rather than re-calculated.  We use the [++snoc](urbit-docs/language/hoon/reference/stdlib/2b#snoc) gate to append a noun to a `list`.
    ...
    (As in an earlier code example, `(add index 1)` can be replaced by the Nock increment rune, `.+` [dotlus](urbit-docs/language/hoon/reference/rune/dot#-dotlus).)

    This version is a little more complicated to compare using a diagram because of the [trap](urbit-docs/glossary/trap), but yields something like this:
    ...
    The program can be improved somewhat again by appending to the head of the cell (rather than using `++snoc`).  This builds a list in a backwards order, so we apply the [++flop](urbit-docs/language/hoon/reference/stdlib/2b#flop) gate to flip the order of the list before we return it.
...
- Change the order of the aspects of the call in such a way that the compiler can produce a more [tail-recursive](https://en.wikipedia.org/wiki/Tail_call) program.
    ...
    The above code should look familiar.  We are still building a gate that takes one argument a `@ud` unsigned decimal integer `n`.  The `|-` here is used to create a new gate with one [arm](urbit-docs/glossary/arm) `$` and immediately call it. As before, think of `|-` as the recursion point.
