...
Every [noun](urbit-docs/glossary/noun) in Urbit is a either an [atom](urbit-docs/glossary/atom) or a [cell](urbit-docs/glossary/cell).  Since a cell has only two elements, a head and a tail, we can derive that everything is representable as a [_binary tree_](https://en.wikipedia.org/wiki/Binary_tree).  We can draw this layout naturally:
...
For instance, if we produce a cell in the [Dojo](urbit-docs/glossary/dojo)
...
- Produce a [generator](urbit-docs/glossary/generator) called `list.hoon` which accepts a single `@ud` number `n` as input and produces a list of numbers from `1` up to (but not including) `n`. For example, if the user provides the number `5`, the program will produce: `~[1 2 3 4]`.
...
(Notice that all values are converted to the specified [aura](urbit-docs/glossary/aura), in this case the empty aura.)

A `list` is built with the `list` mold.  A `list` is actually a _mold builder_, a [gate](urbit-docs/glossary/gate) that produces a gate.  This is a common design pattern in Hoon.  (Remember that a [mold](urbit-docs/glossary/mold) is a type and can be used as an enforcer:  it attempts to convert any data it receives into the given structure, and crashes if it fails to do so.) Lists are commonly written with a shorthand `~[]`:
...
A null-terminated tuple is almost the same thing as a list.  (That is, to Hoon all lists are null-terminated tuples, but not all null-terminated tuples are lists.  This gets rather involved in subtleties, but you should cast a value as `(list @)` or another type as appropriate whenever you need a `list`.  See also [++limo](urbit-docs/language/hoon/reference/stdlib/2b#limo) which explicitly marks a null-terminated tuple as a `list`.)

##  Addressing Limbs

Everything in Urbit is a binary tree.  And all code in Urbit is also represented as data.  One corollary of these facts is that we can access any arbitrary part of an expression, gate, [core](urbit-docs/glossary/core), whatever, via addressing (assuming proper permissions, of course).  (In fact, we can even hot-swap parts of cores, which is how [wet gates](urbit-docs/courses/hoon-school/R-metals#wet-gates) work.)

There are three different ways to access values:

1. [Numeric addressing](urbit-docs/courses/hoon-school/G-trees#numeric-addressing) is useful when you know the address, rather like knowing a house's street address directly.
2. [Positional addressing](urbit-docs/courses/hoon-school/G-trees#positional-addressing-(lark-notation)) is helpful when you don't want to figure out the room number, but you know how to navigate to the value.  This is like knowing the directions somewhere even if you don't know the house number.
3. [Wing addressing](urbit-docs/courses/hoon-school/G-trees#wings) is a way of attaching a name to the address so that you can access it directly.
...
A [tape](urbit-docs/glossary/tape) is one way of representing a text message in Hoon.  It is written with double quotes:
...
Lark notation can locate a position in a tree of any size.  However, it is most commonly used to grab the head or tail of a cell, e.g. in the _type spear_ (on which [more later](urbit-docs/courses/hoon-school/M-typecheck)):
...
When lark expressions resolve to the part of the subject containing an [arm](urbit-docs/glossary/arm), they don't evaluate the arm.  They simply return the indicated noun fragment of the subject, as if it were a leg.
...
One can also identify a resource by a label, called a [wing](urbit-docs/glossary/wing).  A wing represents a depth-first search into the current [subject](urbit-docs/glossary/subject) (context).  A wing is a limb resolution path into the subject. A wing expression indicates the path as a series of limb expressions separated by the `.` character. E.g.,
...
Hoon doesn't have variables like other programming languages do; it has [faces](urbit-docs/glossary/face).  Faces are like variables in certain respects, but not in others.  Faces play various roles in Hoon, but most frequently faces are used simply as labels for legs.
...
If you run into `^$`, don't go look for a `^$` ketbuc rune:  it's matching the outer `$` buc arm.  `^$` is one way of setting up a `%=` [centis](urbit-docs/language/hoon/reference/rune/cen#-centis) loop/recursion of multiple cores with a `|-` [barhep](urbit-docs/language/hoon/reference/rune/bar#--barhep) [trap](urbit-docs/glossary/trap) nested inside of a `|=` [bartis](urbit-docs/language/hoon/reference/rune/bar#-bartis) gate, for instance.

Solution #1 in the [Rhonda Numbers](urbit-docs/language/hoon/examples/rhonda) tutorial in the Hoon Workbook illustrates using `^` ket to skip `$` buc matches.
...
Now we're equipped to go back and examine the syntax of the `%=` [centis](urbit-docs/language/hoon/reference/rune/cen#-centis) rune we have been using for recursion:  it _resolves a wing with changes_, which in this particular case means that it takes the `$` (default) arm of the [trap](urbit-docs/glossary/trap) core, applies certain changes, and re-evaluates the expression.
...
The `$()` syntax is the commonly-used irregular form of the `%=` [centis](urbit-docs/language/hoon/reference/rune/cen#-centis) rune.
...
It's far more common to just use a trap, but you will see `$` buc used to manipulate a [core](urbit-docs/glossary/core) in many in-depth code instances.

### Expanding the Runes
 
`|=` [bartis](urbit-docs/language/hoon/reference/rune/bar#-bartis) produces a gate. It actually expands to
...
where `=|` [tisbar](urbit-docs/language/hoon/reference/rune/tis#-tisbar) means to add its sample to the current subject with the given [face](urbit-docs/glossary/face).

Similarly, `|-` [barhep](urbit-docs/language/hoon/reference/rune/bar#--barhep) produces a [core](urbit-docs/glossary/core) with one arm `$`.  How could you write that in terms of `|%` and `++`?

### Example:  Number to Digits

- Compose a generator which accepts a number as `@ud` unsigned decimal and returns a [list](urbit-docs/glossary/list) of its digits.
...
A more idiomatic solution would use the `^` ket infix to compose a cell and build the list from the head first.  (This saves a call to [++weld](urbit-docs/language/hoon/reference/stdlib/2b#weld).)
...
(Notice that we apply `@t` as a [mold](urbit-docs/glossary/mold) gate rather than using the tic notation.  This is because `^` ket is a rare case where the order of evaluation of operators would cause the intuitive writing to fail.)

- Extend the above [generator](urbit-docs/glossary/generator) so that it accepts a cell of type and value (a `vase` as produced by the `!>` [zapgar](urbit-docs/language/hoon/reference/rune/zap#-zapgar) rune).  Use the type to determine which number base the digit string should be constructed from; e.g. `+num2dig !>(0xdead.beef)` should yield `~['d' 'e' 'a' 'd' 'b' 'e' 'e' 'f']`.
...
- The [++flop](urbit-docs/language/hoon/reference/stdlib/2b#flop) function reverses the order of the elements (exclusive of the `~`):
...
- The [++sort](urbit-docs/language/hoon/reference/stdlib/2b#sort) function uses a `list` and a comparison function (like [++lth](urbit-docs/language/hoon/reference/stdlib/1a#lth)) to order things:
...
- The [++snag](urbit-docs/language/hoon/reference/stdlib/2b#snag) function takes an index and a `list` to grab out a particular element (note that it starts counting at zero):
...
- The [++gulf](urbit-docs/language/hoon/reference/stdlib/2b#gulf) function spans between two numeric values (inclusive of both):
...
- The [++reap](urbit-docs/language/hoon/reference/stdlib/2b#reap) function repeats a value many times in a `list`:
...
- The [++roll](urbit-docs/language/hoon/reference/stdlib/2b#roll) function takes a list and a [gate](urbit-docs/glossary/gate), and accumulates a value of the list items using that gate. For example, if you want to add or multiply all the items in a list of atoms, you would use roll:
...
Once you have a `list` (including a [tape](urbit-docs/glossary/tape)), there are a lot of manipulation tools you can use to extract data from it or modify it:

- The [++lent](urbit-docs/language/hoon/reference/stdlib/2b#lent) function takes `[a=(list)]` and gets the number of elements (length) of the list
- The [++find](urbit-docs/language/hoon/reference/stdlib/2b#find) function takes `[nedl=(list) hstk=(list)]` and locates a sublist (`nedl`, needle) in the list (`hstk`, haystack)
- The [++snap](urbit-docs/language/hoon/reference/stdlib/2b#snap) function takes `[a=(list) b=@ c=*]` and replaces the element at an index in the list (zero-indexed) with something else
- The [++scag](urbit-docs/language/hoon/reference/stdlib/2b#scag) function takes `[a=@ b=(list)]` and produces the first _a_ elements from the front of the list
- The [++slag](urbit-docs/language/hoon/reference/stdlib/2b#slag) function takes `[a=@ b=(list)]` and produces all elements of the list including and after the element at index _a_
...
- Write a gate that takes in a list `a` and returns `%.y` if `a` is a palindrome and `%.n` otherwise.  You may use the [++flop](urbit-docs/language/hoon/reference/stdlib/2b#flop) function.
