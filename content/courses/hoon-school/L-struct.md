...
[++maps](../../language/hoon/reference/stdlib/2o#map) are a versatile way to store and access data, but they are far from the only useful pattern. `++map`s were documented in [the previous module](K-doors).
...
- [++mop](../../language/hoon/reference/zuse/2m#mop) ordered maps are discussed in [the App School guides](../app-school).
