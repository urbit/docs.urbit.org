...
We'll talk in more detail about the various kinds of type-checking that can occur at arm evaluation [when we discuss type polymorphism](R-metals).
...
By now you've used the `|=` [bartis](../../language/hoon/reference/rune/bar#-bartis) rune to define several [gates](../../glossary/gate).  This rune is used to produce a _dry gate_, which has different type-checking and type-inference properties than a _wet gate_ does.  We won't explain the distinction until [a later module](R-metals)—for now, just keep in mind that we're only dealing with one kind of gate (albeit the more common kind).
