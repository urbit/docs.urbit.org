...
(If you see a `|*` [bartar](../../language/hoon/reference/rune/bar#-bartar) rune in the code, it's similar to a `|=` [bartis](../../language/hoon/reference/rune/bar#-bartis), but produces what's called a [_wet gate_](R-metals).)
...
We saw `.^` [dotket](../../language/hoon/reference/rune/dot#-dotket) for the first time in [the previous module](O-subject), where we learned that it performs a _peek_ or [scry](../../glossary/scry) into the state of an Arvo [vane](../../glossary/vane).  Most of the time this functionality is used to ask `%c` [Clay](../../glossary/clay) or `%g` [Gall](../../glossary/gall) for information about a path, [desk](../../glossary/desk), [agent](../../glossary/agent), etc.  In this case, `(cat 3 %c %y)` is a fancy way of collocating the two `@tas` terms into `%cy`, a Clay file or directory lookup.  The type of this lookup is `+$arch`, and the location of the file or directory is given by `arg` from the sample.
