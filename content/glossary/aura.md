...
**Auras** are types of [atoms](urbit-docs/glossary/atom). A generic atom (`@`) is a non-negative decimal integer. Auras allow such atoms to be defined more specifically, such as `@t` for little-endian UTF-8 strings, `@ux` for hexadecimal and `@p` for a [ship](urbit-docs/glossary/ship) name like `~sampel-palnet`. Auras do three things:
...
- [Aura reference](urbit-docs/language/hoon/reference/auras): Additional information about auras.
- [Hoon School: syntax lesson](urbit-docs/courses/hoon-school/B-syntax): Includes details of atoms and auras.
