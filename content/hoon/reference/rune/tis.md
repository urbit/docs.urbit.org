...
Technically the `=.` rune doesn't change the subject. It creates a new subject just like the old one except for a changed value at `p`. Note that the mutation uses [`%_` ("cencab")](cen.md#_-cencab) "BROKEN_ANCHOR", so the type at `p` doesn't change. Trying to change the value type results in a `nest-fail`.
