...
Computes the result of `2^a`, where `a` is a block size (see [$bloq](broken-reference)), producing an atom.
...
`a` is a block size (see [`$bloq`](broken-reference)).

`b` is a `list` of length-value pairs, `p` and `q`:

* `p` is a [step](broken-reference).
* `q` is a `@`.
...
* `b` is a [step](broken-reference).
* `c` is a [step](broken-reference).
...
`a` is an atom slice specifier (see [`$bite`](broken-reference)), which is a block size (see [`$bloq`](broken-reference)) with optional block count.
...
`boz` is a block size with optional block count (see [`$bloq`](broken-reference)).
...
`a` is a [$bloq](broken-reference) (block size).

`[b c d]` where:

* `b` is a [step](broken-reference) specifying the number of `bloq`s to offset.
* `b` is a [step](broken-reference) specifying the number of `bloq`s to replace.
* `d` is the donor atom.
...
`b` is a `bloq`. (see [`$bloq`](broken-reference))
