...
> (~(bif by a) %b)
[ l=[n=[p=%e q=5] l=~ r=~]
  r=[n=[p=%d q=4] l=~ r=[n=[p=%c q=3] l=[n=[p=%a q=1] l=~ r=~] r=~]]
]

> `[(map @tas @) (map @tas @)]`(~(bif by a) %b)
[ [n=[p=%e q=5] l=~ r=~]
  [n=[p=%d q=4] l=~ r=[n=[p=%c q=3] l={[p=%a q=1]} r={}]]
]
```
...
against the old signature therefore fail silently. For example, `(~(bif by a) b+2)`
now splits on the noun `[%b 2]` and returns a pair still containing `[p=%b q=2]`.
