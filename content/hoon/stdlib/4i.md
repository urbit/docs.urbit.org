...
`+iny` is an alias for [`+inde`](4f.md#inde); see there for details.

#### Accepts

`.ope` and `.end` are `$rule`s matching the block's opening and closing
delimiters, passed together as a cell; `.sef` is a `$rule` applied to the
contents. The call shape is `(iny [ope end] sef)`.
...
```
> `tape`(scan "\"\"\"\0afoo\0abar\0a\"\"\"" (iny [(jest '"""') (jest '"""')] (star ;~(pose prn (just `@`10)))))
"foo\0abar"
```
