...
A door that contains arms that are used to scramble two `$atom`s, `top` and `bot`. Used especially in the phonetic base to disguise the relationship between a planet and its star.
...
`top` & `bot` are `$atom`s, and are the sample of `+mu`.
