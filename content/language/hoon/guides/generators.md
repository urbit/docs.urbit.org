...
`%ask` generators return `sole-effect`s.  For more information on these, consult the [guide on command-line agents](urbit-docs/userspace/apps/guides/cli-tutorial).
...
The [`+dbug` agent](urbit-docs/userspace/apps/examples/dbug) is invoked against an agent to display internal state.

Any app can implement generators to wrap raw pokes (see [`%ahoy`](urbit-docs/userspace/apps/examples/ahoy) for instance).
