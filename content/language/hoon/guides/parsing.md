...
**Note:** For JSON printing/parsing and encoding/decoding, see the [JSON Guide](urbit-docs/language/hoon/guides/json-guide).
...
These functions are used to build `rule`s (i.e. parsers), and thus are often called rule-builders. For a complete list of parser builders, see [4f: Parsing (Rule-Builders)](urbit-docs/language/hoon/reference/stdlib/4f), but also the more specific functions in [4h: Parsing (ASCII Glyphs)](urbit-docs/language/hoon/reference/stdlib/4h), [4i: Parsing (Useful Idioms)](urbit-docs/language/hoon/reference/stdlib/4i), [4j: Parsing (Bases and Base Digits)](urbit-docs/language/hoon/reference/stdlib/4j), [4l: Atom Parsing](urbit-docs/language/hoon/reference/stdlib/4l).

### [`+just`](urbit-docs/language/hoon/reference/stdlib/4f/#just)
...
### [`+jest`](urbit-docs/language/hoon/reference/stdlib/4f/#jest)
...
### [`+shim`](urbit-docs/language/hoon/reference/stdlib/4f/#shim)
...
### [`+next`](urbit-docs/language/hoon/reference/stdlib/4f/#next)
...
### [`+cold`](urbit-docs/language/hoon/reference/stdlib/4f/#cold)
...
One common scenario where `+cold` sees play is when writing [command line interface (CLI) apps](urbit-docs/userspace/apps/guides/cli-tutorial). We usher the reader there to find an example where `+cold` is used.

### [`+less`](urbit-docs/language/hoon/reference/stdlib/4f/#less)
...
### [`+knee`](urbit-docs/language/hoon/reference/stdlib/4f/#knee)
...
For additional information including examples see [4g: Parsing (Outside Caller)](urbit-docs/language/hoon/reference/stdlib/4g).

### Parsing `tape`s

[`+scan`](urbit-docs/language/hoon/reference/stdlib/4g/#scan) takes in a `tape` and a `rule` and attempts to parse the `tape` with the
`rule`.
...
[`+rust`](urbit-docs/language/hoon/reference/stdlib/4g/#rust) is the unitized version of `+scan`.
...
[Recall from Hoon School](urbit-docs/courses/hoon-school/E-types) that `cord`s are atoms with the aura `@t` and are typically used to represent strings internally as data, as atoms are faster for the computer to work with than `tape`s, which are `list`s of `@tD` atoms. [`+rash`](urbit-docs/language/hoon/reference/stdlib/4g/#rash) and [`+rush`](urbit-docs/language/hoon/reference/stdlib/4g/#rush) are for parsing atoms, with `+rash` being analogous to `+scan` and `+rush` being analogous to `+rust`. Under the hood, `+rash` calls `+scan` after converting the input atom to a `tape`, and `+rush` does similary for `+rust`.

## Parser modifiers

The standard library provides a number of gates that take a `rule` and produce a new modified `rule` according to some process. We call these _parser modifiers_. These are documented among the [parser builders](urbit-docs/language/hoon/reference/stdlib/4f).

### [`+ifix`](urbit-docs/language/hoon/reference/stdlib/4f/#ifix)
...
`+pal` and `+par` are shorthand for `(just '(')` and `(just ')')`, respectively. All ASCII glyphs have counterparts of this sort, documented [here](urbit-docs/language/hoon/reference/stdlib/4h).

### [`+star`](urbit-docs/language/hoon/reference/stdlib/4f/#star) {#star}
...
### [`+cook`](urbit-docs/language/hoon/reference/stdlib/4f/#cook)
...
Building complex parsers from simpler parsers is accomplished in Hoon with the use of two tools: the monadic applicator rune [`;~`](urbit-docs/language/hoon/reference/rune/mic/#-micsig) and [parsing combinators](urbit-docs/language/hoon/reference/stdlib/4e). First we introduce a few combinators, then we examine more closely how `;~` is used to chain them together.
...
### [`+plug`](urbit-docs/language/hoon/reference/stdlib/4e/#plug)
...
### [`+pose`](urbit-docs/language/hoon/reference/stdlib/4e/#pose)
...
### [`+glue`](urbit-docs/language/hoon/reference/stdlib/4e/#glue)
...
### [`;~`](urbit-docs/language/hoon/reference/rune/mic/#-micsig) {#-micsig}
...
Functions for parsing numbers are documented in [4j: Parsing (Bases and Base Digits)](urbit-docs/language/hoon/reference/stdlib/4j). In particular, [`dem`](urbit-docs/language/hoon/reference/stdlib/4i/#dem) is a `rule` for parsing decimal numbers.
...
Here, [`+prn`](urbit-docs/language/hoon/reference/stdlib/4i/#prn) is a `rule` used to parse any printable character, and [`+easy`](urbit-docs/language/hoon/reference/stdlib/4f/#easy) is a `rule` that always returns a constant (`~` in this case) regardless of the input.

Thus some special sauce is required, the [`+knee`](urbit-docs/language/hoon/reference/stdlib/4f/#knee) function.
