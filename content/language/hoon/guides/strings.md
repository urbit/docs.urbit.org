...
Hoon has a system for writing more elaborate functional parsers, but that is not touched on here. Instead, see the [Parsing](urbit-docs/language/hoon/guides/parsing) guide. Hoon also has a type for UTF-32 strings, but those are rarely used and not discussed in this document.
...
A [`cord`](urbit-docs/language/hoon/reference/stdlib/2q#cord) has an aura of `@t`. It denotes UTF-8 text encoded in an atom, little-endian. That is, the first character in the text is the least-significant byte. A cord may contain any UTF-8 characters, there are no restrictions.
...
A [`knot`](urbit-docs/language/hoon/reference/stdlib/2q#knot) has an aura of `@ta`, and is a subset of a [`cord`](#cord). It allows lower-case letters, numbers, and four special characters: Hyphen, tilde, underscore and period. Its restricted set of characters is intended to be URL-safe.
...
A [`term`](urbit-docs/language/hoon/reference/stdlib/2q#term) has an aura of `@tas`, and is a subset of a [`knot`](#knot). It only allows lower-case letters, numbers, and hyphens. Additionally, the first character cannot be a hyphen or number. This is a very restricted text atom, and is intended for naming data structures and the like.
...
This means you cannot rely on mere aura-casting if you need the text to conform to the specified aura's restrictions. Instead, there are a couple of function in the standard library to check text aura validity: [`+sane`](urbit-docs/language/hoon/reference/stdlib/4b#sane) and [`+sand`](urbit-docs/language/hoon/reference/stdlib/4b#sane).
...
A [`tape`](urbit-docs/language/hoon/reference/stdlib/2q#tape) is the other main string type in hoon. Rather than a single atom, it's instead a list of individual `@tD` characters (the `D` specifies a bit-length of 8, see the [Auras](urbit-docs/language/hoon/reference/auras#bitwidth) documentation for details). The head of the list is the first character in the string.
...
Tapes can easily be converted to cords and vice versa. There are two stdlib functions for this purpose: [`+crip`](urbit-docs/language/hoon/reference/stdlib/4b#crip) and [`+trip`](urbit-docs/language/hoon/reference/stdlib/4b#trip). The former converts a `tape` to a `cord` and the latter does the opposite. For example:
...
It's common to encode atoms in cords or knots, particularly when constructing a [scry](urbit-docs/system/kernel/arvo/guides/scry) [`path`](urbit-docs/language/hoon/reference/stdlib/2q#path) or just a `path` in general. There are two main functions for this purpose: [`+scot`](urbit-docs/language/hoon/reference/stdlib/4m#scot) and [`+scow`](urbit-docs/language/hoon/reference/stdlib/4m#scow). The former produces a `knot`, and the latter produces a `tape`. Additionally, there are two more functions for encoding `path`s in cords and tapes respectively: [`+spat`](urbit-docs/language/hoon/reference/stdlib/4m#spat) and [`+spud`](urbit-docs/language/hoon/reference/stdlib/4m#spud).
...
For decoding atoms of particular auras encoded in cords, there are three functions: [`+slat`](urbit-docs/language/hoon/reference/stdlib/4m#slat), [`+slav`](urbit-docs/language/hoon/reference/stdlib/4m#slav), and [`+slaw`](urbit-docs/language/hoon/reference/stdlib/4m#slaw). Additionally, there is [`+stab`](urbit-docs/language/hoon/reference/stdlib/4m#stab) for decoding a cord to a path.
...
- [Parsing](urbit-docs/language/hoon/guides/parsing) - A guide to writing fully-fledged functional parsers in hoon.
  
- [Developer Blog, "What Every Hooner Should Know About Text on Urbit"](https://urbit.org/blog/text-overview)

- [Auras](urbit-docs/language/hoon/reference/auras) - Details of auras in hoon.

- [stdlib 2b: List logic](urbit-docs/language/hoon/reference/stdlib/2b) - Standard library functions for manipulating lists, which are useful for dealing with tapes.

- [stdlib 2q: Molds and Mold-builders](urbit-docs/language/hoon/reference/stdlib/2q) - Several text types are defined in this section of the standard library.

- [stdlib 4b: Text processing](urbit-docs/language/hoon/reference/stdlib/4b) - Standard library functions for manipulating and converting tapes and strings.

- [stdlib 4m: Formatting functions](urbit-docs/language/hoon/reference/stdlib/4m) - Standard library functions for encoding and decoding atom auras in strings.
