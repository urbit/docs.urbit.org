...
A [`$vase`](urbit-docs/language/hoon/reference/stdlib/4o#vase) is a pair of [`$type`](urbit-docs/language/hoon/reference/stdlib/4o#type) and [`$noun`](urbit-docs/language/hoon/reference/stdlib/2q#noun), where the type describes the noun. They're used all over Urbit to represent data whose type we can't know ahead of time. This often comes up when being asked to compile and run other Hoon code. It's also used to store data that could be any type, but where we want to know the type, so we tag the value with its type to form a vase.

- The [Arvo](urbit-docs/glossary/arvo) kernel uses vases to build itself and run [vanes](urbit-docs/glossary/vane) (kernel modules).
- The [Gall](urbit-docs/glossary/gall) vane used to use vases to run userspace [agents](urbit-docs/glossary/agent), and probably will again in the future.
- The Ford build system (in the [Clay](urbit-docs/glossary/clay) vane) uses vases to build Hoon code and store typed files.
- The [Dojo](urbit-docs/glossary/dojo) shell uses vases to compile and run shell expressions and to run generators.
- The [Spider](urbit-docs/glossary/spider) agent uses vases to run [threads](urbit-docs/glossary/thread) (scripts written in an IO monad).
...
The programmer can name parts of a type, including the type of the subject. Once part of a type has a name, Hoon code can refer to any named field within that type. Since the subject of any Hoon expression always has a known type at compile time, a reference (a [wing](urbit-docs/glossary/wing), like `foo.+<.bar.|3.baz`) to a field within that subject is compiled to a Nock 0 operation (subtree lookups) at a known constant [axis](urbit-docs/language/hoon/reference/stdlib/2q#axis) (tree slot).
...
An `%atom` type describes a set of numbers. The `p=term` in an atom type is its [aura](urbit-docs/glossary/aura), a name for a kind of value that can be stored as an atom -- `%da` for date, `%ux` for hexadecimal number, `%t` for text, etc. The `q=(unit @)` is either `~`, meaning any value, or `[~ value]`, in which case this type has only a single member. The type `%foo` refers to the set whose only instance is the atom `%foo`.
...
All executable Hoon code is found in a [core](urbit-docs/glossary/core). This core stores a map from [arm](urbit-docs/glossary/arm) name (like an OOP getter function) to result type, along with some other information about the core. If the core is a [door](urbit-docs/glossary/door) (like an OOP object) or a [gate](urbit-docs/glossary/gate) (like an anonymous function), then slot 6 (the head of the tail) of the core is a "sample" slot, which is overwritten with instance data or function argument, respectively. Whether this core expects a sample, and if so, what the sample's type is, is represented in the `$type` data structure for cores of that type.
...
The vase operations form a relatively simple algebra. This algebra can be thought of as a dynamically typed programming language. Each value in the language is a vase (a dynamically typed datum), and the basic operations are [`+slap`](urbit-docs/language/hoon/reference/stdlib/5c#slap) and [`+slop`](urbit-docs/language/hoon/reference/stdlib/5c#slop).
...
The primary introduction form for a vase is the [`!>`](urbit-docs/language/hoon/reference/rune/zap#-zapgar) rune, which produces a vase of its input. A vase can also be constructed manually as a cell whose head nests in `$type`, like `[[%atom %ud ~] 3]`.

### Elimination Forms

An elimination form for a vase is something that converts a vase to a statically typed value.

One unsafe elimination form is the [`!<`](urbit-docs/language/hoon/reference/rune/zap#-zapgal) rune, which takes a mold and a value and (unsafely) converts the value to a typed value. The more traditional elimination form has no syntactic support, but involves pattern-matching on the type data structure, e.g. dispatching based on whether the type is atom, cell, or something else, and coercing the value to a specific type using a mold function.
