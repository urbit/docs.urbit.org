...
A pair of a [`mark`](#mark) and a [`vase`](stdlib/4o#vase) (type-value pair). These are extensively used for passing data around between vanes and agents.
...
Type-wise, a `wire` is the same as a [`path`](stdlib/2q#path); a `list` of [`knot`](stdlib/2q#knot)s with the syntax of `/foo/bar/baz`. While a `path` is typically used in requests to denote a scry or subscription endpoint, a `wire` is used for responses.
