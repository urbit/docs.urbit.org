...
wrong fanout for a rune. For example, `%+` ([**cenlus**](/language/hoon/reference/rune/cen#-cenlus),
...
can use the `~!` rune ([**sigzap**](/language/hoon/reference/rune/sig#-sigzap)) to print the type of any hoon in your stack trace.
...
`mint-vain` means this hoon is never executed. `mint-lost` means there's a case in a `?-` ([**wuthep**](/language/hoon/reference/rune/wut#--wuthep)) that isn't handled.
...
([**sigbar**](/language/hoon/reference/rune/sig#-sigbar)):
...
`~&` ([**sigpam**](/language/hoon/reference/rune/sig#-sigpam)) is Hoon's debugging printf.
...
`~?` ([**sigwut**](/language/hoon/reference/rune/sig#-sigwut)), which prints only if a condition is
true:
