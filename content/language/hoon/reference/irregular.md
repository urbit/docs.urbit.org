...
When in doubt, you can use the [`!,` zapcom](rune/zap#-zapcom) rune to determine the AST to which Hoon parses an expression.
...
| Form | Regular Form |
| ---- | ------------ |
| `_foo` | [`$_`](rune/buc#_-buccab), normalizes to an example |
| `foo=bar` | [`$=`](rune/buc#-buctis), wraps a face around a value |
| `?(%foo %bar %baz)` | [`$?`](rune/buc#-bucwut), forms a type union |
| `(fun a b c)` | [`%:`](rune/cen#-cencol), calls a gate with n arguments |
| `~(arm core arg)` | [`%~`](rune/cen#-censig), pulls an arm in a door |
| `foo(x 1, y 2, z 3)` | [`%=`](rune/cen#-centis), resolve a wing with changes |
| `[a b c]` | [`:*`](rune/col#-coltar) or [`$:`](rune/buc#-buccol), constructs _n_-tuple in normal mode or its structure in structure mode |
| `~[a b c]` | [`:~`](rune/col#-colsig), constructs null-terminated list |
| `+(42)` | [`.+`](rune/dot#-dotlus), increments with Nock 4 |
| `=(a b)` | [`.=`](rune/dot#-dottis), tests for equality wiht Nock 5 |
| `` `foo` bar`` | [`^-`](rune/ket#--kethep), typecasts by explicit type label |
| `foo=bar` | [`^=`](rune/ket#-kettis), binds name to value |
| `*foo` | [`^*`](rune/ket#-kettar), bunts (produces default mold value) |
| `,foo` | [`^:`](rune/ket#-ketcol), produces “factory” gate for type |
| `:(fun a b c d)` | [`;:`](rune/mic#-miccol), calls binary function as _n_-ary function |
| `foo:bar` | [`=<`](rune/tis#-tisgal), composes two expressions, inverted |
| `\|(foo bar baz)` | [`?\|`](rune/wut#-wutbar), logical OR (loobean) |
| `&(foo bar baz)` | [`?&`](rune/wut#-wutpam), logical AND (loobean) |
| `!foo` | [`?!`](rune/wut#-wutzap), logical NOT (loobean) |
...
See [%sand](rune/constants#warm) for other irregular definitions of atoms.
...
- `>a b c<` produces a [tank](stdlib/2q/#tank) of the output of the contents (wrapped in cell if more than 1 item), formatted in pretty-print.
...
- `<a b c>` produces a [tape](stdlib/2q/#tape) of the tank above (ie `<1 2 3>` is same as `~(ram re >1 2 3<)`).
