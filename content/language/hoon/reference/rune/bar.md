...
Because gates are also doors, you can call them the same way. To call the gate `foo` as a door, instead of `(foo baz)` we would write `~($ foo baz)`. This is an irregular form for `%~($ foo baz)`, [%~](urbit-docs/language/hoon/reference/rune/cen#-censig).
...
> `$(...)` expands to `%=($ ...)` (["centis"](urbit-docs/language/hoon/reference/rune/cen#-centis)).
...
See [this discussion of core variance models](urbit-docs/language/hoon/reference/advanced)
...
The `|@` rune is just like the `|%` rune except that instead of producing a 'dry' core, it produces a 'wet' one. This allows for type polymorphism of its arms, using 'genericity'. See [Advanced types](urbit-docs/language/hoon/reference/advanced).
...
See this [discussion of the core variance model](urbit-docs/language/hoon/reference/advanced).
