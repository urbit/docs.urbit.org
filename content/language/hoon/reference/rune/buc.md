...
Structures are abstract syntax trees for `type`s (see the documentation on [basic](../basic) and [advanced](../advanced) types for the precise definition of `type`). Structures are compile-time values of `type` which at runtime may be used to produce a 'mold'.
...
You should make sure that the product type of `p` nests under `q`. You can check the default value of some structure (custom type) `r` with `*r`. (See the [`^*` rune](ket#-kettar).)
