...
See [`tome`](urbit-docs/language/hoon/reference/stdlib/4o#tome) in the Hoon standard library.
...
Under the hood, `+*` gets compiled as [`=*`'s](urbit-docs/language/hoon/reference/rune/tis#-tistar) (see here for more discussion on deferred expressions). `+* foo bar` rewrites each `++` arm beneath it in the core to include `=* foo bar`. For example, the interpreter sees the Nock compiled from this Hoon expression
