...
[`+biff`](../stdlib/2a/#biff) is the unit monad's implementation of monadic bind. That is to say, it takes a unit `a` and a gate `b` that accepts a noun that produces a unit, and extracts the value from `a` to pass as a sample to `b`.
...
A [`marl`](../stdlib/5e#marl), i.e., a list of [`manx`](../stdlib/5e#manx). A `manx` is a noun that represents a single XML node.
...
`;=` is a [Sail](../../guides/sail) rune. Sail is a part of Hoon used for creating and operating on nouns that represent XML nodes. With the appropriate rendering pipeline, a Sail document can be used to generate a static website.
