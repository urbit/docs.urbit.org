...
This produces the [`$hoon`](urbit-docs/language/hoon/reference/stdlib/4o#hoon) AST of expression `q`. The first argument, `p`, is always an example of the `$hoon` type, typically just the `*hoon` bunt value, and is used for type inference. The reason for `p` is just to handle transitions if the `$hoon` type changes.
...
Wrap a noun in its type (form a [`vase`](urbit-docs/language/hoon/reference/stdlib/4o#vase)).
...
In Hoon, a dynamic type is a static type compiled at runtime. This type-noun cell is generally called a [`vase`](urbit-docs/language/hoon/reference/stdlib/4o#vase).
...
Extracts a [`vase`](urbit-docs/language/hoon/reference/stdlib/4o#vase) to the given mold if its type nests.
...
The first field, `p`, must be an example of the [`$type`](urbit-docs/language/hoon/reference/stdlib/4o#type) type, typically just `*type` (the bunt of `$type`). The `p` argument is just so transitions can be handled if the `$type` type changes.
