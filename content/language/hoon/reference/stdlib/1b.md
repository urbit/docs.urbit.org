...
Tests whether the tree address `a` is in the head or the tail of a [`noun`](/TODO-GLOSSARY/noun).
Produces the constant [`atom`](/TODO-GLOSSARY/atom) `%2` if it is within the head (subtree `+2`), or
the constant [`atom`](/TODO-GLOSSARY/atom) `%3` if it is within the tail (subtree `+3`).

#### Accepts

`a` is an [`atom`](/TODO-GLOSSARY/atom).

#### Produces

A constant [`atom`](/TODO-GLOSSARY/atom).
...
Computes the tree address of [`atom`](/TODO-GLOSSARY/atom) `a` within either the head (`+2`) or tail
(`+3`) of a [`noun`](/TODO-GLOSSARY/noun).
...
Running `(mas 7)` in the [`Dojo`](/TODO-GLOSSARY/dojo) will return `3`, because address `+3` is what
...
`b` is an [`atom`](/TODO-GLOSSARY/atom).
...
Running `(peg 3 4)` in the [`Dojo`](/TODO-GLOSSARY/dojo), for example, will return `12`. Looking at
