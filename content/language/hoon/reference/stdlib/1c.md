...
A [`core`](../../../../glossary/core) with one arm, `$`--the empty name--which transforms a sample noun into a product noun. If used dryly as a type, the subject must have a sample type of `*`.
...
A mold generator. Produces a tuple of a [`list`](1c#list) of `item` and the mold of `state`.
