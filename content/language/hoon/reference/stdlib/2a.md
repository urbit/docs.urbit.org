...
Applies a function `b` that produces a [`unit`](1c#unit) to the unwrapped value of [`unit`](1c#unit) `a` (`u.a`). If `a` is empty, `~` is produced.

#### Accepts

`a` is a [`unit`](1c#unit).

`b` is a function that accepts a [`noun`](../../../../glossary/noun) and produces a [`unit`](1c#unit).
...
Applies a function `b` to the value (`u.a`) of a [`unit`](1c#unit) `a`, producing a [`unit`](1c#unit). Used when you want a function that does not accept or produce a [`unit`](1c#unit) to both accept and produce a [`unit`](1c#unit).
...
Replaces an empty [`unit`](1c#unit) `b` with the product of a called [`trap`](../../../../glossary/trap) `a`. If the [`unit`](1c#unit) is not empty, then the original [`unit`](1c#unit) is produced.
...
Produces `~` if either `a` or `b` are empty. Otherwise, produces a [`unit`](1c#unit) whose value is a cell of the values of two input [`units`](1c#unit) `a` and `b`.
...
Combine two [`units`](1c#unit) with function

Applies a binary function `c`--which does not usually accept or produce a [`unit`](1c#unit)-- to the values of two [`units`](1c#unit), `a` and `b`, producing a [`unit`](1c#unit).
...
Compose two [`units`](1c#unit) with function

Applies a binary function `c`--which does not usually accept [`units`](1c#unit)-- to the values of two [`units`](1c#unit), `a` and `b`, producing a [`unit`](1c#unit).
...
Either a [`noun`](../../../../glossary/noun) `b` or the unwrapped value of [`unit`](1c#unit) `a`.
...
Accepts function `a` and produces a function that accepts [`unit`](1c#unit) `b` to which it applies `a`. Used when you want a function that does not accept or produce a [`unit`](1c#unit) to both accept and produce a [`unit`](1c#unit).
...
Accepts two [`units`](1c#unit) `a` and `b` whose values are expected to be equivalent. If either is empty, then the value of the other is produced. If neither are empty, it asserts that both values are the same and produces that value. If the assertion fails, `++mate` crashes with `'mate'` in the stack trace.
...
Takes any [`noun`](../../../../glossary/noun) `a` and produces a [`unit`](1c#unit) with the value set to `a`.
