...
`a` is a block size (see [`$bloq`](1c#bloq)).
...
`a` is an atom slice specifier (see [`$bite`](1c#bite)), which is a block size (see [`$bloq`](1c#bloq)) with optional block count.
...
`boz` is a block size with optional block count (see [`$bloq`](1c#bloq)).
...
`a` is a [$bloq](1c#bite) (block size).

`[b c d]` where:

- `b` is a [step](1c#step) specifying the number of `bloq`s to offset.
- `b` is a [step](1c#step) specifying the number of `bloq`s to replace.
- `d` is the donor atom.
