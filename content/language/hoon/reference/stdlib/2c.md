...
`a` is an atom slice specifier (see [`$bite`](language/hoon/reference/stdlib/1c#bite)), which is a block size (see [`$bloq`](language/hoon/reference/stdlib/1c#bloq)) with optional block count.
...
`boz` is a block size with optional block count (see [`$bloq`](language/hoon/reference/stdlib/1c#bloq)).
