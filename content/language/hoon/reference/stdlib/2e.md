...
Implementation of the 32-bit [MurmurHash3](https://en.wikipedia.org/wiki/MurmurHash#Algorithm) non-cryptographic hash algorithm. Takes `syd` as the seed, `len` as the key length in [blocks](/language/hoon/reference/stdlib/2c) of size 3, and `key` as the key, producing an atom.
