...
If `a` and `b` are both atoms, `dor` is equivalent to [`lte`](urbit-docs/language/hoon/reference/stdlib/1a#lte). If they're cells, `dor` recurses on the heads, and then if the heads are the same it checks the tails.
...
[`mug`](urbit-docs/language/hoon/reference/stdlib/2e#mug) is the the 31-bit nonzero FNV-1a hash algorithm.
