...
If `a` and `b` are both atoms, `dor` is equivalent to [`lte`](language/hoon/reference/stdlib/1a#lte). If they're cells, `dor` recurses on the heads, and then if the heads are the same it checks the tails.
