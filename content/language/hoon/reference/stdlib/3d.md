...
Produces a 128-bit atom by performing the [bitwise XOR](language/hoon/reference/stdlib/2d) on the first and last halves of the 256-bit salted hash `shas`.
...
Produces a 128-bit atom by hashing a noun `yux` with the [`+shaf`](#shaf) function. If that noun is a cell, then it is passed to the [`jam`](language/hoon/reference/stdlib/2p#jam) function to produce an atom to be hashed.
...
Produces an atom by using SHA-256 plus a salt input. The [bitwise XOR](language/hoon/reference/stdlib/2d) is performed on salt `sal` and the product of atom `ruz` hashed with SHA-256. The product of that logical operation is then itself hashed with SHA-256.
