...
Keep in mind that `+grub:dejs-soft:format` and `+fist:dejs-soft:format` defined here (and subsequently referred to as simply `fist` and `grub`) are different to `+grub:dejs:format` and `+fist:dejs:format` for the [+dejs](urbit-docs/language/hoon/reference/zuse/2d_6) functions, which simply return `noun`s rather than `unit`s.
