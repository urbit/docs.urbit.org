...
Cold state is the assignment of core hierarchies to labels. It is accumulated as a side effect, as computations hinted with `%fast` are evaluated. (This hint is emitted by Hoon's [`~%` sigcen](../../hoon/reference/rune/sig#-sigcen) and [`~/` sigfas](../../hoon/reference/rune/sig#-sigfas) runes).
