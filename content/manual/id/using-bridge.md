...
weight = 7
aliases = ["/docs/manual/getting-started/using-bridge/"]
+++
...
and do not wish to [factory reset](https://developers.urbit.org/TODO-GLOSSARY/reset), check the box labeled
...
With that keyfile in hand, you can now exit Bridge and continue to the guide to [install the Urbit binary](https://urbit.org/manual/getting-started/).
...
[OTAs](https://developers.urbit.org/TODO-GLOSSARY/ota-updates) to them as well. To accomplish this, enter `|ota ~sponsor %kids` in dojo, where `~sponsor` is the `@p` of your new sponsor.
