...
- Running an Urbit ship with the ordinary runtime [from the command line](/manual/getting-started/cli).
...
You can use the key and moon name in the same installation flow from the [Command line installation](/manual/getting-started/cli) guide, following the same scheme as for booting a planet. That scheme is:
