...
XOR](/language/hoon/reference/stdlib/2d) on the first and last halves of the 256-bit
salted hash `shas`.
...
[`jam`](/language/hoon/reference/stdlib/2p#jam) function to produce an atom to be
hashed.
...
XOR](/language/hoon/reference/stdlib/2d) is performed on salt `sal` and the product
...
Produces an atom by hashing an atom `ruz` with SHA-256. Another atom, `len`, is
