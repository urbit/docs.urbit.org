...
[Ecliptic.eth](https://etherscan.io/address/ecliptic.eth) holds the business logic for the ledger kept by `Azimuth.eth`. It may be modified by [galaxy vote](urbit-docs/glossary/upgrade). This determines things such as what the various proxies are capable of, how keys are changed, or verifying that a request is valid.

`Ecliptic.eth` uses external contracts such as [Azimuth.eth](urbit-docs/system/identity/reference/azimuth-eth) and [Polls](https://github.com/urbit/azimuth/blob/master/contracts/Polls.sol) for data storage so that it can easily be replaced in case the logic needs to be changed without affecting the data. These data contracts are owned by `Ecliptic.eth`, and this ownership is passed to the new Ecliptic contract whenever it is replaced. Thus it is advised for clients to not store Ecliptic's contract address directly, but instead ask the `Azimuth.eth` contract for its `owner` attribute to ensure that transactions are sent to the latest Ecliptic contract. Alternatively, the [ENS](https://ens.domains/) name `Ecliptic.eth` will always resolve to the latest Ecliptic.

You can read about [Urbit's first upgrade](https://github.com/urbit/azimuth/pull/35) to Ecliptic, which occurred in the summer of 2021, [here](https://urbit.org/blog/first-contract). The [second](https://github.com/urbit/azimuth/pull/43) occurred later in the year and consisted of several small modifications to ready the PKI for the introduction of [naive rollups](urbit-docs/system/identity/concepts/layer2).
...
There are currently [28 functions](#write) which may be called to write to the Ecliptic, and [17 functions](#read) to read data from the Ecliptic. Many of these have a corresponding [layer 2 action](urbit-docs/system/identity/reference/l2-actions), and/or can be performed using [Bridge](https://urbit.org/using/id/using-bridge). We note these facts where applicable.
...
These functions are available to each owner of a [`Point`](urbit-docs/system/identity/reference/azimuth-eth#points), and a subset of them are available to its [proxies](#proxies). All of these actions may be performed from Bridge.
...
Configure `_point` with network public keys `_encryptionKey`, `_authenticationKey`, and corresponding `_cryptoSuiteVersion`, incrementing the `Point`'s [`keyRevisionNumber`](urbit-docs/system/identity/reference/azimuth-eth#points) if the keys have changed and `continuityNumber` number if `_discontinuous` is set to true (see [Life and Rift](urbit-docs/system/identity/concepts/life-and-rift)).
...
Transfer `_point` to `_target`, clearing all permissions data and keys if `_reset` is true. `_reset` set to true makes this transaction a [breach](https://urbit.org/using/id/guide-to-resets), and thus this action increments the [`continuityNumber`](urbit-docs/system/identity/reference/azimuth-eth#points) of `_point`, and usually the `keyRevisionNumber` as well (see [Life and Rift](urbit-docs/system/identity/concepts/life-and-rift)).

Requirements:

- `:msg.sender` must be either `_point`'s current owner, authorized to transfer `_point`, or authorized to transfer the current owner's points (i.e. is listed as an ERC-721 operator in [`operators`](urbit-docs/system/identity/reference/azimuth-eth#other)).
...
Unlike all other layer 1 actions, layer 1 sponsors may use a layer 1 `%detach` on a layer 2 sponsee. See the [Layer 2](urbit-docs/system/identity/concepts/layer2#sponsorship) section for more detail. The detach action available in Bridge is a layer 2 action, so a layer 1 detach must be done [manually](https://etherscan.io/address/ecliptic.eth#writeContract).
...
Most of these are functions only available to galaxies. They are related to [voting](urbit-docs/glossary/voting). As voting does not occur on layer 2, there are no corresponding layer 2 actions for poll actions.
...
This returns the deposit address for [layer 2](urbit-docs/system/identity/concepts/layer2), which is `0x1111111111111111111111111111111111111111`. Ships sent to this address are controlled on layer 2 instead of via Ecliptic.
...
Returns the address of the [Azimuth.eth](urbit-docs/system/identity/reference/azimuth-eth) contract: `0x223c067f8cf28ae173ee5cafea60ca44c335fecb`.
...
Beginning in 2019, stars may spawn at most 1024 planets. This limit doubles every subsequent year until the maximum is reached. However, this limit is not currently implemented on [Layer 2](urbit-docs/system/identity/concepts/layer2).
