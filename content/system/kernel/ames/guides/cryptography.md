...
[$peer-state](/system/kernel/ames/reference/data-types#peer-state), inside which the [$symmetric-key](/system/kernel/ames/reference/data-types#symmetric-key) (an atom which nests under
`@uw`) is utilized for encrypting all Ames packets shared between the two ships.
The `symmetric-key` is derived using [`+shar:ed:crypto`](/language/hoon/reference/cryptography#ed) found in `sys/zuse.hoon`,
...
is generated with
[`+sign:as:crub`](/language/hoon/reference/cryptography#sign-as) found in
`sys/zuse.hoon`.
...
[lead](/language/hoon/reference/advanced#dry-polymorphism-and-core-nesting-rules)
...
[`crub:crypto`](/language/hoon/reference/cryptography#crub) for more details on
