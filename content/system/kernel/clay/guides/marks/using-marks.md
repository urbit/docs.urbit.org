...
In the last document, [Writing Marks](urbit-docs/system/kernel/clay/guides/marks/writing-marks), we walked through writing `mark` files and touched on how Clay handles them. They needn't just be left to background vane processes though, you can also use them yourself in your code.
...
You can either use these by `%pass`ing Clay a [%warp task](urbit-docs/system/kernel/clay/reference/tasks#warp) with the appropriate `care`, or else with a [Clay scry](urbit-docs/system/kernel/clay/reference/scry). In the examples here we've used the latter.
