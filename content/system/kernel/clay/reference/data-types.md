...
- `rom`: the state for all local desks. It consists of a `duct` to [Dill](urbit-docs/system/kernel/dill) and a collection of `desk`s.
...
- `hun`: the duct we use to send messages to [Dill](urbit-docs/system/kernel/dill) to display notifications of filesystem changes. Only `%note` `%gift`s should be produced along this `duct`. This is set by the `%init` `move`.
...
See the [scry reference](urbit-docs/system/kernel/clay/reference/scry) for more details.
...
See the [Strategies](urbit-docs/system/kernel/clay/guides/using#strategies) section of "Using Clay" for further details of their meaning.
