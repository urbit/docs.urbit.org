...
A buc scry with a path of `/sweep` will check the global ford cache for refcount errors. It returns a `(list [need=@ud have=@ud leak])`, where a [`leak`](urbit-docs/system/kernel/clay/reference/data-types#leak) is a Ford cache key used internally by Clay.
...
[`rang`](urbit-docs/system/kernel/clay/reference/data-types#rang) from Clay's state.
...
See the [`$rave:clay`](urbit-docs/system/kernel/clay/reference/data-types#rave) data type entry for more details of the `$rave` structure.
...
A buc scry with a path of `/flow` will return the global build cache. The type returned is a [`$flow:clay`](urbit-docs/system/kernel/clay/reference/data-types#flow).
...
A buc scry with a path of `/domes` will return a [`cone`](urbit-docs/system/kernel/clay/reference/data-types#cone) containing the `dome`s and associated metadata for all desks, foreign and local.
...
A buc scry with a path of `/tire` will return the `rock:tire:clay` for all domestic desks, which is a `(map desk [=zest wic=(set weft)])`. The [`zest`](urbit-docs/system/kernel/clay/reference/data-types#zest) specifies whether apps on the desk are running or suspended. The `wic` set contains the `weft`s (kernel versions) of any queued updates.
...
This will return the [yaki:clay](urbit-docs/system/kernel/clay/reference/data-types#yaki) of the specified commit. It takes a [tako:clay](urbit-docs/system/kernel/clay/reference/data-types#tako).

Example:

Here we scry the [dome:clay](urbit-docs/system/kernel/clay/reference/data-types#dome) for `%`, get the latest `tako` and the do a `%s` scry for the `yaki` in question.
...
This will return the [page:clay](urbit-docs/system/kernel/clay/reference/data-types#page) of some file. It takes a [lobe:clay](urbit-docs/system/kernel/clay/reference/data-types#lobe).
...
This will return the `@uvI` (256-bit) content hash of the specified commit. It takes a [`tako`](urbit-docs/system/kernel/clay/reference/data-types#tako).

Example:

Here we grab the [`dome`](urbit-docs/system/kernel/clay/reference/data-types#dome) for `%` with a `%v` scry, get the latest [`tako`](urbit-docs/system/kernel/clay/reference/data-types#tako) and then do a `%s` `%hash` scry for it.
...
This will return the most recent revision number of a `desk` that has been fully downloaded. The type it returns is a [`cass`](urbit-docs/system/kernel/clay/reference/data-types#cass). The `case` in the `beak` must be a revision number rather than a date. You can just provide a case of `1` since it returns the latest regardless. If we have nothing for the specified `desk`, this will just return the bunt of a `cass` like `cass=[ud=0 da=~2000.1.1]`.
...
A scry with a care of `%v` will return the entire state of a `desk` as a [`dome`](urbit-docs/system/kernel/clay/reference/data-types#dome).
...
A scry with a `care` of `%w` will return the revision number and date of a given `case`. The type returned is a [`cass`](urbit-docs/system/kernel/clay/reference/data-types#cass) like `[ud=@ud da=@da]` where `ud` is the revision number and `da` is the date.
...
An `arch` is a `[fil=(unit lobe:clay) dir=(map @ta ~)]`. The `fil` will contain the [`lobe`](urbit-docs/system/kernel/clay/reference/data-types#lobe) hash if it's a file, otherwise it will be null. The `dir` will contain a map of the files and directories it contains, otherwise it will be null.
