...
This document details all the `task`s you're likely to use to interact with Clay, as well as the `gift`s you'll receive in response. Most sections have a corresponding practical example in the [Examples](urbit-docs/system/kernel/clay/examples/examples) document. Many of the types referenced are detailed in the [Data Types](urbit-docs/system/kernel/clay/reference/data-types) document. It may also be useful to look at the `++ clay` section of `/sys/lull.hoon` in Arvo where these `task`s, `gift`s and data structures are defined.
...
The `wer` field is the target ship. The `(unit rave)` of the [riff](urbit-docs/system/kernel/clay/reference/data-types#riff) is null to cancel an existing subscription, otherwise the [rave](urbit-docs/system/kernel/clay/reference/data-types#rave) is tagged with one of:
...
The `unit` of the [riot](urbit-docs/system/kernel/clay/reference/data-types#riot) will be null if the target file cannot be found or if a subscription has ended (depending on context). Otherwise it will have a [rant](urbit-docs/system/kernel/clay/reference/data-types#rant) with a `cage` containing the data you requested. Its contents will vary depending on the kind of request and `care`.
...
The `care` of the [mood](urbit-docs/system/kernel/clay/reference/data-types#mood) will determine what you can read and what type of data will be returned. See the [care](urbit-docs/system/kernel/clay/reference/data-types#care) documentation and [scry](urbit-docs/system/kernel/clay/reference/scry) documentation for details on the various `care`s.

The [case](urbit-docs/system/kernel/clay/reference/data-types#case) specifies the `desk` revision and you can use whichever kind you prefer. The `path` will usually be a path to a file or directory like `/gen/hood/hi/hoon` but may be something else depending on the `care`.

#### Example

[See here for an example of using %sing.](urbit-docs/system/kernel/clay/examples/examples#sing)
...
This subscribes to the next version of the specified file. See [here](urbit-docs/system/kernel/clay/reference/data-types#mood) for details of the `mood` structure.
...
[See here for an example of using %next.](urbit-docs/system/kernel/clay/examples/examples#next)
...
[See here for an example of using %mult.](urbit-docs/system/kernel/clay/examples/examples#mult)
...
If the `track` is `%.n`, the `cage` of the `%writ` will contain a [nako](urbit-docs/system/kernel/clay/reference/data-types#nako) with the relevant data for all changes to a desk between what you have and the `case` requested. It is very large and fairly complicated. The `nako` structure is defined in the `clay.hoon` source file itself rather than in `lull.hoon` or elsewhere since you're unlikely to work with it yourself.

The `from` and `to` fields of the [moat](urbit-docs/system/kernel/clay/reference/data-types#moat) specify the range of `case`s for which to subscribe. The range is _inclusive_. It can be specified by date or by revision number, whichever you prefer.
...
[See here for an example of using %many.](urbit-docs/system/kernel/clay/examples/examples#many)
...
[See here for an example of cancelling a subscription.](urbit-docs/system/kernel/clay/examples/examples#cancel-subscription)
...
If the head of the [nori](urbit-docs/system/kernel/clay/reference/data-types#nori) `dit` is `%|`, it's a request to add a label to a commit, and the `nori` looks like `[%| p=@tas q=(unit aeon)]` where `p` is the label and `q` is the [`aeon`](urbit-docs/system/kernel/clay/reference/data-types#aeon) (commit reference). If `q` is null, the label is applied to the latest commit in the desk.

If the head of the `nori` is `%&`, it's a request to add, delete or modify one or more files in the given desk, and looks like `[%& p=soba]`. The [soba](urbit-docs/system/kernel/clay/reference/data-types#soba) in the `nori` is just a list of changes so you can make more than one change in one request. Its `path` is just the path to a file like `/gen/hood/hi/hoon` and the [miso](urbit-docs/system/kernel/clay/reference/data-types#miso) is one of these types of requests:
...
- [%del](urbit-docs/system/kernel/clay/examples/examples#del)
- [%ins](urbit-docs/system/kernel/clay/examples/examples#ins)
- [%mut](urbit-docs/system/kernel/clay/examples/examples#mut)
- [Multiple Changes](urbit-docs/system/kernel/clay/examples/examples#multiple-changes)
...
The [`zest`](urbit-docs/system/kernel/clay/reference/data-types#zest) says whether the desk is running (`%live`), suspended (`%dead`), or suspended pending a kernel-compatible update (`%held`). The `wic` set contains the `weft`s (kernel versions) of any queued updates.
...
A `%zest` `task` suspends or unsuspends a desk. the [`zest`](urbit-docs/system/kernel/clay/reference/data-types#zest) in `liv` is one of:
...
Tombstoning is the deletion of data for old desk revisions. Clay has a single `%tomb` `task`, but its [`clue`](urbit-docs/system/kernel/clay/reference/data-types#clue) has a number of different possible actions:
...
A `%tomb` `task` with a `%lobe` `clue` will tombstone the `page` matching the given [`lobe`](urbit-docs/system/kernel/clay/reference/data-types#lobe). If the `page` in question is used in the current revision of any desks, it will fail. Otherwise, it will be tombstoned globally.
...
A `%tomb` `task` with a `%pick` `clue` will perform garbage collection, tombstoning any data that should be tombstoned according to current tombstoning policy ([`norm`](urbit-docs/system/kernel/clay/reference/data-types#norm)s).
...
A `%tomb` `task` with a `%norm` `clue` will set the default tombstoning policy for the given `desk` and `ship`. A [`norm`](urbit-docs/system/kernel/clay/reference/data-types#norm) is an `(axal ?)`. An `axal` is like a recursive `arch`, and is defined in `arvo.hoon`. The `?` says whether to *keep* the given file or directory. You may want to look at the `+of` axal engine in `arvo.hoon` for constructing and manipulating the `norm`.
...
A `%tomb` `task` with a `%worn` `clue` is like [`%norm`](#norm---default-policy), except it only applies to a specific commit for a ship/desk. The [`tako`](urbit-docs/system/kernel/clay/reference/data-types#tako) denotes the commit to apply the policy.
...
A `%tomb` `task` with a `%seek` `clue` will attempt to retrieve missing, tombstoned data and integrate it into Clay's object store. The [`cash`](urbit-docs/system/kernel/clay/reference/data-types#cash) is a reference to a commit on the given ship/desk as either a [`tako`](urbit-docs/system/kernel/clay/reference/data-types#tako) or a [`case`](urbit-docs/system/kernel/clay/reference/data-types#case).
...
[See here for an example of using %boat.](urbit-docs/system/kernel/clay/examples/examples#boat)
...
[See here for an example of using %mont.](urbit-docs/system/kernel/clay/examples/examples#mont)
...
[See here for an example of using %ogre.](urbit-docs/system/kernel/clay/examples/examples#ogre)
...
[See here for an example of using %dirk.](urbit-docs/system/kernel/clay/examples/examples#dirk)
...
The `germ` specifies the merge strategy. You can refer to the [Strategies](urbit-docs/system/kernel/clay/guides/using#strategies) section of the [Using Clay](urbit-docs/system/kernel/clay/guides/using) document for details of each `germ`.
...
[See here for an example of using %merg.](urbit-docs/system/kernel/clay/examples/examples#merg)
...
[See here for an example of using %perm.](urbit-docs/system/kernel/clay/examples/examples#perm)
...
[See here for an example of using %cred.](urbit-docs/system/kernel/clay/examples/examples#cred)
...
[See here for an example of using %crew.](urbit-docs/system/kernel/clay/examples/examples#crew)
...
[See here for an example of using %crow.](urbit-docs/system/kernel/clay/examples/examples#crow)
...
[See here for examples of requests to foreign ships.](urbit-docs/system/kernel/clay/examples/examples#foreign-ships)
