...
`p` is columns, `q` is rows. This structure is passed to Dill by the runtime in a [%blew](tasks#blew) `task` whenever the dimensions of the terminal changes.
...
A `$belt` is passed to Dill in a [%belt](tasks#belt) `task` by the runtime whenever there is input, such as a user typing in the console. This is only used between the terminal client and Dill, a [$dill-belt](#dill-belt) is used between Dill and Arvo.
...
These are a subset of Dill's `task`s which can be wrapped in a `%flog` `task`. See the [API Reference](tasks) document for details of each of these `task`s.
...
A subset of [Dill's `task`s](tasks#session-tasks) for interacting with a particular session.
...
This type is used in the [`%shot`](tasks#shot) wrapper `task`.

See the [Session Tasks](tasks#session-tasks) entry in the API reference for more details of these `task`s.

---

## `$told`

A subset of [Dill's `task`s](tasks#session-tasks) for basic text printing.
...
See the [Told Tasks](tasks#told-tasks) entry in the API reference for more details of these `task`s.
