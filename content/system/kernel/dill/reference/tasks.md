...
This `task` is used only once, when Arvo first enters the [adult stage](urbit-docs/system/kernel/arvo#structural-interface-core). Dill is technically the first vane to be activated, via the `%boot` `task`, which then sends Jael (considered the "true" first vane) the `%dawn` or `%fake` `task` wrapped in the `%boot` `task`. Jael then goes on to call `%init` `task`s for other vanes (including Dill).

`lit` specifies whether to boot in lite mode. `p` is either a [%dawn](urbit-docs/system/kernel/jael/reference/tasks#dawn) or [%fake](urbit-docs/system/kernel/jael/reference/tasks#fake) `task:jael`. `%dawn` is for an ordinary boot and `%fake` is for booting a fake ship.
...
A `%flog` `task` takes a [$flog](urbit-docs/system/kernel/dill/reference/data-types#flog) as its argument. A `$flog` is a subset of Dill's `task`s.
...
A `$told` is either a [`%crud`](#crud), [`%talk`](#talk) or [`%text`](#text) task. See the [`$told`](urbit-docs/system/kernel/dill/reference/data-types#dill-belt) entry in the data types reference for more details.
...
A `$session-task` is one of these `task`s: [`%belt`](#belt), [`%blew`](#blew), [`%flee`](#flee), [`%hail`](#hail), [`%open`](#open), [`%shut`](#shut), [`%view`](#view). See the [`$session-task`](urbit-docs/system/kernel/dill/reference/data-types#session-task) entry in the data types reference.
...
This `task` toggles verbose mode for all of Arvo, which is located here since Dill is the vane that prints errors. To be precise, `%verb` toggles the laconic bit `lac` in the [Arvo state](urbit-docs/system/kernel/arvo#the-state) by passing a `%verb` `waif` to Arvo.
...
This subset of Dill's `task`s are defined in the [`$session-task` type](urbit-docs/system/kernel/dill/reference/data-types#session-task).
...
The [$belt](urbit-docs/system/kernel/dill/reference/data-types#belt) in `p` contains the input such as which key was pressed. Dill will convert the `$belt` into a [$dill-belt](urbit-docs/system/kernel/dill/reference/data-types#dill-belt) and `%poke` it into the session handler agent for the session in question.
...
The [$blew](urbit-docs/system/kernel/dill/reference/data-types#blew) specifies the new dimensions.

Dill will convert the `$blew` into a `%rez` [$dill-belt](urbit-docs/system/kernel/dill/reference/data-types#dill-belt) and `%poke`s the session handler (typically `drum`) with it.
...
Dill converts a `%hail` `task` into a `%hey` [$dill-belt](urbit-docs/system/kernel/dill/reference/data-types#dill-belt) and `%poke`s the session handler (typically `%drum`) with it.
...
Dill will poke every agent listed in  `q` (local or remote) with a [`%yow` `$dill-belt`](urbit-docs/system/kernel/dill/reference/data-types#dill-belt), to let it know it's been connected. It will also `%watch` the agent `p` for [`$dill-blit`](urbit-docs/system/kernel/dill/reference/data-types#dill-blit)s in `%fact`s with a `%dill-blit` `mark` on the `path` `/dill/[ses]` where `ses` is the session specified in the [`%shot`](#shot) wrapper.
...
See the [`$blit`](urbit-docs/system/kernel/dill/reference/data-types#blit) entry in the data type reference for details of what it can contain.
...
See the [`$blit`](urbit-docs/system/kernel/dill/reference/data-types#blit) entry in the data type reference for more details.
...
This subset of Dill `task`s are for printing things. They are defined in the [`$told` type](urbit-docs/system/kernel/dill/reference/data-types#told).
