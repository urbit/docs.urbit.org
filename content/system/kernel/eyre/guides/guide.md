...
General documentation of the `task`s and methods described here are available in the [External API Reference](urbit-docs/system/kernel/eyre/reference/external-api-ref) document and the [Internal API Reference](urbit-docs/system/kernel/eyre/reference/tasks) document.
...
Here we'll look at a practical example of Eyre's channel system. You can refer to the [Channels](urbit-docs/system/kernel/eyre/reference/external-api-ref#channels) section of the [External API Reference](urbit-docs/system/kernel/eyre/reference/external-api-ref) document for relevant details.
...
The data will be a JSON array containing a [poke](urbit-docs/system/kernel/eyre/reference/external-api-ref#poke) `action`:
...
We'll do this in the same way as the initial poke, except this time it will be a [subscribe](urbit-docs/system/kernel/eyre/reference/external-api-ref#subscribe) `action`:
...
Back in the event stream, we'll see a positive watch ack as a [subscribe](urbit-docs/system/kernel/eyre/reference/external-api-ref#watch-ack) `response`, meaning the subscription has been successful:
...
Now we'll try trigger an event on our event stream. In fakezod's Landscape, create a new chat channel named "test". You should see the `add-graph` `graph-update` come through on our channel in a [diff](urbit-docs/system/kernel/eyre/reference/external-api-ref#diff) `response`:
...
All events we receive must be `ack`ed so Eyre knows we've successfully received them. To do this we'll send an [ack](urbit-docs/system/kernel/eyre/reference/external-api-ref#ack) `action` which specifies the `event-id` of the event in question - `2` in this case:
...
When we're finished, we can unsubscribe from `graph-store` `/update`. We do this by sending Eyre a [unsubscribe](urbit-docs/system/kernel/eyre/reference/external-api-ref#unsubscribe) `action`, and specify the request ID of the original `subscribe` `action` in the `subscription` field - `2` in our case:
...
Finally, let's close the channel itself. We can do this simply by sending Eyre a [delete](urbit-docs/system/kernel/eyre/reference/external-api-ref#delete-channel) `action`:
...
Here we'll look at performing scries through Eyre. You can refer to the [Scry](urbit-docs/system/kernel/eyre/reference/external-api-ref#scry) section of the [External API Reference](urbit-docs/system/kernel/eyre/reference/external-api-ref) document for relevant details.
...
You can refer to the [%connect](urbit-docs/system/kernel/eyre/reference/tasks#connect) section of the [Internal API Reference](urbit-docs/system/kernel/eyre/reference/tasks) document for relevant details.
...
Now, first we need to bind a URL to our app. In the `++ on-poke` arm, our agent will send a [%connect](urbit-docs/system/kernel/eyre/reference/tasks#connect) `task` to Eyre when poked with `%bind`:
...
You can refer to the [%serve](urbit-docs/system/kernel/eyre/reference/tasks#serve) section of the [Internal API Reference](urbit-docs/system/kernel/eyre/reference/tasks) document for relevant details.
...
The return type of the generator must be [$simple-payload:http](urbit-docs/system/kernel/eyre/reference/data-types#simple-payloadhttp). If you look at our example generator you'll see it meets these requirements.
...
Here we'll look at approving and rejecting a CORS origin by passing Clay a [%approve-origin](urbit-docs/system/kernel/eyre/reference/tasks#approve-origin) `task` and [%reject-origin](urbit-docs/system/kernel/eyre/reference/tasks#reject-origin) `task` respectively.
...
Now if we scry for the [approved](urbit-docs/system/kernel/eyre/reference/scry#cors-approved) CORS `set`:
...
If we scry for the [rejected](urbit-docs/system/kernel/eyre/reference/scry#cors-rejected) CORS `set`:
...
Now if we scry for the [requests](urbit-docs/system/kernel/eyre/reference/scry#cors-requests) CORS `set`:
