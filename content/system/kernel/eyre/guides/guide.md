...
General documentation of the `task`s and methods described here are available in the [External API Reference](../reference/external-api-ref) document and the [Internal API Reference](../reference/tasks) document.
...
It's advisable to have a read through the [External API Reference](../reference/external-api-ref) before going through these examples.

### Authenticating

You must have a valid session cookie in order to use Eyre's interfaces (such as the channel system or scry interface). If your HTTP client is served from your ship, your browser will automatically add the session cookie it obtained upon login, so there's no need to worry about authentication in practice. If your client does not run in the browser or is not served by your ship, [authenticating](../reference/external-api-ref#authentication) with your web login code (which can be obtained by running `+code` in the dojo) is necessary.
...
Here we'll look at a practical example of Eyre's channel system. You can refer to the [Channels](../reference/external-api-ref#channels) section of the [External API Reference](../reference/external-api-ref) document for relevant details.
...
The data will be a JSON array containing a [poke](../reference/external-api-ref#poke) `action`:
...
Eyre will respond with an HTTP status code of 200 and a `content-type` of `text/event-stream`, indicating an SSE (Server Sent Event) stream. It will also send us any pending events on the channel - in this case the poke ack as a [poke](../reference/external-api-ref#poke-ack) `response` for our original poke:
...
We'll do this in the same way as the initial poke, except this time it will be a [subscribe](../reference/external-api-ref#subscribe) `action`:
...
Back in the event stream, we'll see a positive watch ack as a [subscribe](../reference/external-api-ref#watch-ack) `response`, meaning the subscription has been successful:
...
Now we'll try trigger an event on our event stream. In fakezod's Landscape, create a new chat channel named "test". You should see the `add-graph` `graph-update` come through on our channel in a [diff](../reference/external-api-ref#diff) `response`:
...
All events we receive must be `ack`ed so Eyre knows we've successfully received them. To do this we'll send an [ack](../reference/external-api-ref#ack) `action` which specifies the `event-id` of the event in question - `2` in this case:
...
When we're finished, we can unsubscribe from `graph-store` `/update`. We do this by sending Eyre a [unsubscribe](../reference/external-api-ref#unsubscribe) `action`, and specify the request ID of the original `subscribe` `action` in the `subscription` field - `2` in our case:
...
Finally, let's close the channel itself. We can do this simply by sending Eyre a [delete](../reference/external-api-ref#delete-channel) `action`:
...
Here we'll look at performing scries through Eyre. You can refer to the [Scry](../reference/external-api-ref#scry) section of the [External API Reference](../reference/external-api-ref) document for relevant details.
...
You can refer to the [%connect](../reference/tasks#connect) section of the [Internal API Reference](../reference/tasks) document for relevant details.
...
Now, first we need to bind a URL to our app. In the `++ on-poke` arm, our agent will send a [%connect](../reference/tasks#connect) `task` to Eyre when poked with `%bind`:
...
You can refer to the [%serve](../reference/tasks#serve) section of the [Internal API Reference](../reference/tasks) document for relevant details.
...
The return type of the generator must be [$simple-payload:http](../reference/data-types#simple-payloadhttp). If you look at our example generator you'll see it meets these requirements.
...
Here we'll look at approving and rejecting a CORS origin by passing Clay a [%approve-origin](../reference/tasks#approve-origin) `task` and [%reject-origin](../reference/tasks#reject-origin) `task` respectively.
...
Now if we scry for the [approved](../reference/scry#cors-approved) CORS `set`:
...
If we scry for the [rejected](../reference/scry#cors-rejected) CORS `set`:
...
Now if we scry for the [requests](../reference/scry#cors-requests) CORS `set`:
