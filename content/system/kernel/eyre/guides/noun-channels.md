...
If you are not familiar with low-level Eyre channel mechanics, please have a read through the [Eyre guide](urbit-docs/system/kernel/eyre/guides/guide) first.
...
...and the body contains the [`++jam`](urbit-docs/language/hoon/reference/stdlib/2p#jam) of a `list` of `$channel-request`s with [`@uw`](urbit-docs/language/hoon/reference/auras#table-of-auras) base64 encoding.
...
See the [`$channel-event`](urbit-docs/system/kernel/eyre/reference/data-types#channel-event) entry in the data type reference for more details.
