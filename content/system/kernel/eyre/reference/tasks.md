...
Many of the types referenced are detailed in the [Data Types](data-types) document. It may also be useful to look at the `+eyre` section of `/sys/lull.hoon` in Arvo where these `task`s, `gift`s and data structures are defined.
...
The `secure` field says whether it's over HTTPS. The `address` is the IP address from which the request originated. The [$request:http](data-types#requesthttp) is the HTTP request itself containing the method, URL, headers, and body.
...
The [$binding](data-types#binding) contains a URL path and optional domain through which the agent will be able to take HTTP requests. The `app` is just the name of the Gall agent to bind. Note that if you bind a URL path of `/foo`, Eyre will also match `/foo/bar`, `/foo/bar/baz`, etc.

If an agent is bound in Eyre using this method, HTTP requests to the bound URL path are poked directly into the agent. The `cage` in the poke have a `%handle-http-request` `mark` and a `vase` of `[@ta inbound-request:eyre]` where the `@ta` is a unique `eyre-id` and the [$inbound-request](data-types#inbound-request) contains the HTTP request itself.
...
The first `fact`'s `cage` must have a `mark` of `%http-response-header` and a `vase` containing a [$response-header:http](data-types#response-headerhttp) with the HTTP status code and headers of the response.
...
See the [Agents: Direct HTTP](../guides/guide#agents-direct-http) section of the [Guide](../guides/guide) document for an example.
...
The `binding` contains the URL path and optional domain through which the generator will take HTTP requests. The [$generator](data-types#generator) specifies the `desk`, the `path` to the generator in Clay, and also has a field for arguments. Note that the passing of specified arguments to the generator by Eyre is not currently implemented, so you can just leave it as `~`.

The bound generator must be a gate within a gate and the type returned must be a [$simple-payload:http](data-types#simple-payloadhttp).
...
The `?` says whether the HTTP request contained a valid session cookie and the [$request:http](data-types#requesthttp) contains the request itself.
...
See the [Generators](../guides/guide#generators) section of the [Guide](../guides/guide) document for an example.
...
The [$binding](data-types#binding) is the URL path and domain of the binding you want to delete.
...
The [$origin](data-types#origin) is a CORS origin like `http://foo.example` you want to approve.
...
See the [Managing CORS Origins](../guides/guide#managing-cors-origins) section of the [Guide](../guides/guide) document for an example.
...
The [$origin](data-types#origin) is a CORS origin like `http://foo.example` you want want to reject.
...
The `entry` field is a [`$cache-entry`](data-types#cache-entry) in a `unit`. If the unit is null, the specified `url` will be unbound and the cache entry removed. If non-null, the given `entry` will be added to the cache (or updated if the binding already exists).

Each time the entry for a URL path is changed, its revision number will be incremented.

See the [`$cache-entry`](data-types#cache-entry) entry in Eyre's type reference for more details of the entry itself.
