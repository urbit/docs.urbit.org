...
Note that multiparty encrypted scry (specified with a true `secret`) should only be used when you know the publisher expects it (i.e, as part of their application protocol). Otherwise, the two-party [Ames `%chum` task](urbit-docs/system/kernel/ames/reference/tasks#chum) should be used.
...
Gall translates agent [`task:agent`](urbit-docs/system/kernel/gall/reference/data-types#taskagent)s emitted by agents into `%deal` tasks, as well as requests from over the network. This `task` is kernel-level only, it cannot be used directly from userspace.

Its fields are:

- `p`: A `sock`, a `(pair ship ship)`, the sending and receiving ships.
- `q`: The source agent.
- `r`: A [`deal`](urbit-docs/system/kernel/gall/reference/data-types#deal) is either a [`task:agent`](urbit-docs/system/kernel/gall/reference/data-types#taskagent) or a `%raw-poke`. This is the request itself.
...
The agent specified in `dude` will be suspended. Note it is usually better to suspend agents with a [`%rein`](urbit-docs/system/kernel/clay/reference/tasks#rein---force-apps) `task` to Clay rather than an `%idle` `task` to Gall.
...
See the [`load`](urbit-docs/system/kernel/gall/reference/data-types#load) entry in the type reference for more details of the datastructure in this `task`.
...
This sets verbosity flags for Gall. Currently there's only one [`verb`](urbit-docs/system/kernel/gall/reference/data-types#verb), `%odd`, which prints messages for unusual error cases. This overwrites the existing verbosity settings: an empty list will turn all verbosity flags off.
