...
Iris itself only has three structures defined in `lull.hoon`, but it also uses `$http` structures which are shared between Iris and Eyre. The `$http` structures are detailed in the [Eyre](urbit-docs/system/kernel/eyre/reference/data-types#http) Data Types documentation.
...
This structure is used in a [%request task](urbit-docs/system/kernel/iris/reference/tasks#request) to specify settings for an HTTP request. The `redirects` field specifies the number of redirects to automatically follow before failing, and defaults to 5. The `retries` field specifies the number of times to retry a failed request before giving up, and defaults to 3.
