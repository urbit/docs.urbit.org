...
You may also wish to reference the [Data Types](data-types) document for details of the types referenced here, and the [Examples](../examples/examples) document for practical examples of using these tasks.
...
This task takes a [$dawn-event](data-types#dawn-event) as its argument.
...
This is what is sent to Jael by `%hood` behind the scenes when you run `|moon`, `|moon-breach` or `|moon-cycle-keys`. The `ship` field is the moon's `@p`. The [$udiff:point](data-types#udiffpoint) will contain the bunt of an [$id:block](data-types#idblock) (since moons aren't registered in Azimuth) and one of the `udiff` actions depending on what you want to do.
...
See the [%public-keys and %nuke](../examples/examples#public-keys-and-nuke) section of the Examples document for an example of using `%nuke` to cancel a `%public-keys` subscription. See the thread in the [%private-keys](../examples/examples#private-keys) example for cancelling a `%private-keys` subscription.
...
See the [%private-keys](../examples/examples#private-keys) section of the Examples document for a practical example.
...
Upon subscription, Jael will immeditely respond with a `%public-keys` gift containing a `%full` `public-keys-result` with the public key for each `life` up until the current one for each `ship` specified in the original task. After than, Jael will send a `%public-keys` gift with either a `%diff` or `%breach` [`$public-keys-result`](data-types#public-keys-result) each time a change occurs for any of the `ship`s to which you're subscribed.

#### Example

See the [%public-keys and %nuke](../examples/examples#public-keys-and-nuke) section of the Examples document for a practical example.
...
Note it's better to use the [`%snub` Ames task](../../ames/reference/tasks#snub) if you want to block packets from ships.
...
The domains returned by a `%turf` task are used as the base for individual galaxy domain names (e.g. from `urbit.org` you get `zod.urbit.org`, `bus.urbit.org`, etc). Jael gets these from Azimuth, then Ames gets them from Jael and passes them to the runtime, which will perform the DNS lookups and give Ames back the galaxy IP addresses. A `%turf` task takes no additional arguments. You're unlikely to use this manually - if you want the current `turf`s you'd likely want to do a [turf scry](scry#turf) instead.
...
document](../examples/examples#turf) for a practical example.
...
See the [%step](../examples/examples#step) section of the Examples document for a practical example.
...
See the [`$public-keys-result`](data-types#public-keys-result) entry in the data types reference for details of the data this gift contains.
