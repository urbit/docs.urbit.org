...
All `++on-init` does is pass Lick a [`%spin`](../reference/tasks#spin) task to create a new `licker.sock` socket.
...
When `++on-poke` receives a poke with a `mark` of `%noun` and data of `%ping`, it passes Lick a [`%spit`](../reference/tasks#spit) task with the same data. Lick will send it on through to our `licker.sock` socket for our Python script. This lets us poke our agent from the Dojo like:
...
`++on-arvo` expects a [`%soak`](../reference/tasks#soak-1) gift from Lick. A `%soak` is primarily a message coming in from the socket, though connection status is also communicated in `%soak`s. The four cases we handle are:
