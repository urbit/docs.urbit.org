...
- [“`u3`: Land of Nouns”](urbit-docs/system/runtime/reference/nouns/) (recommended as supplement to this document)
- [“API overview by prefix”](urbit-docs/system/runtime/reference/api/) (recommended as supplement after this document)
...
Given a Hoon gate, how can a developer produce a matching C jet? Let us illustrate the process using a simple `|%` core.  We assume the reader has achieved facility with both Hoon code and C code.  This tutorial aims to communicate the practical process of producing a jet, and many [`u3` noun concepts](urbit-docs/system/runtime/reference/nouns) are only briefly discussed or alluded to.
...
Similar to how we encountered recursion way back in [Hoon School](urbit-docs/courses/hoon-school/F-cores) to talk about gate mechanics, let us implement a C jet of the `++factorial` example code.  We will call this library `trig` in a gesture to some subsequent functions you should implement as an exercise.  Create a file `lib/trig.hoon` with the following contents:
...
For more information on `u3`, please check out the `u3` summary below or the official documentation at [`“u3`:  Land of Nouns”](urbit-docs/system/runtime/reference/nouns/).
...
- [`~/` sigfas](urbit-docs/language/hoon/reference/rune/sig#-sigfas) registers a jet simply (using defaults).
- [`~%` sigcen](urbit-docs/language/hoon/reference/rune/sig#-sigcen) registers a jet with all arguments specified.
...
2.  **Memory allocation.**  Understanding the memory model, allocation, freeing, and ownership ('transfer' vs 'retain' semantics) is important.  More information is available in [the “Nouns” docs](urbit-docs/system/runtime/reference/nouns).
...
3.  Unit tests rely on using the `-test` thread as covered in [Hoon School](urbit-docs/courses/hoon-school/I-testing) and the [testing guide](urbit-docs/userspace/apps/guides/unit-tests).
...
1.  Reference counting with transfer and retain semantics. (For everything the new developer does outside of real kernel shovel work, one will use transfer semantics.)  These are discussed in [the “Noun” docs](urbit-docs/system/runtime/reference/nouns).

2.  The structure of memory: the loom, with outer and inner roads.  This is discussed in [the “Noun” docs](urbit-docs/system/runtime/reference/nouns).

3.  Many details of C-side atom declaration and manipulation from the `u3` library.  These are discussed in [the API docs](urbit-docs/system/runtime/reference/api).
