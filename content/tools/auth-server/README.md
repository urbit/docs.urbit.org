...
Auth Server is intended to be used via an Eyre airlock. The most common is the `@urbit/http-api` NPM package, which is documented [here](urbit-docs/tools/js-libs/http-api-guide). There are a few airlocks for other languages too, some of which are listed [here](https://github.com/urbit/awesome-urbit#http-apis-airlock). Eyre's interfaces extend Urbit's [poke](urbit-docs/glossary/poke), [scry](urbit-docs/system/kernel/arvo/guides/scry) and [subscription](urbit-docs/system/kernel/arvo/guides/subscriptions) mechanics to HTTP clients. You can read more about Eyre [here](urbit-docs/system/kernel/eyre/guides/guide).

### Actions

There are two actions you can poke into Auth Server: A [`new`](urbit-docs/tools/auth-server/types#new) action to initiate a new request, and a [`cancel`](urbit-docs/tools/auth-server/types#cancel) action to cancel an existing request. The `new` action looks something like this:
...
The [`id`](urbit-docs/tools/auth-server/types#id) field is a random unique ID for the request, and must be a v4 UUID (variant 1, RFC 4122/DCE 1.1).

The fields in the [`request`](urbit-docs/tools/auth-server/types#request) are as follows:
...
There are two main types of updates you'll receive from Auth Server: an [`entry`](urbit-docs/tools/auth-server/types#entry) update and a [`status`](urbit-docs/tools/auth-server/types#status) update. An `entry` update looks like this:
...
It contains the [`id`](urbit-docs/tools/auth-server/types#id) and [`request`](urbit-docs/tools/auth-server/types#request) from the `new` action above, and additionally shows the initial status in the [`result`](urbit-docs/tools/auth-server/types#result) field. This will normally be `"sent"`, unless you set the expiry earlier than *now*, in which case it will immediately be `"expire"` and Auth Server won't bother sending it to the user.

After it's been sent you'll get [`status`](urbit-docs/tools/auth-server/types#status) updates for it when its status changes. A `status` update looks like:
...
Auth Server has a number of different subscription paths, but you'd likely only use one or two. There are two categories of subscription paths: [`/new/...`](urbit-docs/tools/auth-server/subs#new) and [`/init/...`](urbit-docs/tools/auth-server/subs#new). The `/new` paths will start giving you any updates that occur after you subscribe. The `/init` paths will do the same, but they'll also give you initial state. For each of these, there is a path to receive all updates, and there are also sub-paths to filter by [`turf`](urbit-docs/tools/auth-server/types#turf) (domain), [`ship`](urbit-docs/tools/auth-server/types#ship) and [`id`](urbit-docs/tools/auth-server/types#id). Additionally, for each sub-path, you can specify a "since" time, and only receive updates and initial state for requests with `time`s *later* than the one you specify.
...
As described at the beginning, Auth Client checks `<domain>/.well-known/appspecific/org.urbit.auth.json` to verify a request actually comes from the domain it claims. That `.json` file must contain a [`manifest`](urbit-docs/tools/auth-server/types#manifest), which is just an array of [`proof`](urbit-docs/tools/auth-server/types#proof)s. A `proof` looks like:
...
You might also like to look at the Hoon reference for the `++wood` function [here](urbit-docs/language/hoon/reference/stdlib/4b#wood).
