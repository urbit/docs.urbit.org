...
The [`id`](types.md#id) field is a random unique ID for the request, and must be a v4 UUID (variant 1, RFC 4122/DCE 1.1).

The fields in the [`request`](types.md#request) are as follows:
...
It contains the [`id`](types.md#id) and [`request`](types.md#request) from the `new` action above, and additionally shows the initial status in the [`result`](types.md#result) field. This will normally be `"sent"`, unless you set the expiry earlier than *now*, in which case it will immediately be `"expire"` and Auth Server won't bother sending it to the user.

After it's been sent you'll get [`status`](types.md#status) updates for it when its status changes. A `status` update looks like:
...
You might also like to look at the Hoon reference for the `++wood` function [here](../../language/hoon/reference/stdlib/4b.md#wood).
