...
A `$manx` is how an XML node structure is represented in hoon. See [Section 5e](urbit-docs/language/hoon/reference/stdlib/5e#manx) of the standard library reference for details. A `$manx` is what `++de-xml:html` and `++en-xml:html` decode/encode raw XML strings from/to.
...
- **Sail**: this is hoon's native XML syntax. Udon will parse it, execute it, and include the `+$manx`es produced in the resulting document. This means you can embed arbitrary hoon in the document. There is little formal sail documentation, but you can refer to the [`;` (mic) rune reference](urbit-docs/language/hoon/reference/rune/mic) for most of its runes and some rudimentary examples.
