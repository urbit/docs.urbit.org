...
A `jug` is a `(map (set))`.  Because it is a `map`, the `++by` functions can also be utilized with it directly.
...
Deletes the key from `jug`

### Accepts

A `pair` of `jug` and `noun`
...
```hoon
 > =j `(jug @t @ud)`(make:map ~[['a' (make:set ~[1 2 3])] ['b' (make:set ~[4 5 6])]])
 > j
[n=[p='b' q={5 6 4}] l=~ r=[n=[p='a' q={1 2 3}] l={} r={}]]
