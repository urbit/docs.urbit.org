...
In this tutorial we will run a simple "move trace" and use the output to get a picture of what the Arvo kernel proper does during the routine task of setting a timer. Some level of familiarity with the kernel is required for this section, which can be obtained in our [Arvo kernel tutorial](broken-reference).
...
To follow along yourself, boot up a fake `~zod` and enter `|verb` into the dojo and press Enter to enable verbose mode (this is tracked by the laconic bit introduced in the section on [the state](broken-reference)) in the kernel documentation, followed by `-time ~s1` followed by Enter. Your terminal should pretty print a series of `move`s that looks something like this:
...
Here is the line of code in `arvo.hoon`, found in the [section 3bE core](broken-reference), that generated the output:
...
Let's glance at part of the `+jack` arm in `arvo.hoon`, located in the [section 3bE core](broken-reference). This arm is what the Arvo kernel uses to send `card`s, and here we look at the segment that includes `%pass` `move`s.
