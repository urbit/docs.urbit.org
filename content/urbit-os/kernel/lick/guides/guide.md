...
`++on-arvo` expects a [`%soak`](../reference/tasks.md#soak) gift from Lick. A `%soak` is primarily a message coming in from the socket, though connection status is also communicated in `%soak`s. The four cases we handle are:
