...
Most of the poke work takes place through `%ahoy-command`, which checks on the ship state per [Ames](urbit-docs/glossary/ames)’ scheme of `%alien` and `%known` ships, then maintains the agent state by its watchlist.
