...
This branch includes the use of a rare [`=?` tiswut](urbit-docs/language/hoon/reference/rune/tis#-tiswut) conditional leg change and the reversed `=/` tisfas, [`=;` tismic](urbit-docs/language/hoon/reference/rune/tis#-tismic).  There is also some direct compilation of `cord`s taking place:

- [`++sell`](urbit-docs/language/hoon/reference/stdlib/5c#sell) is a `vase` pretty-printer.
- [`++slop`](urbit-docs/language/hoon/reference/stdlib/5c#slop) conses two `vase`s together as a cell. 
- [`++slap`](urbit-docs/language/hoon/reference/stdlib/5c#slap) compiles a Hoon expression and produces a `vase` of the result.
- [`++ream`](urbit-docs/language/hoon/reference/stdlib/5d#ream) parses a `cord` to a Hoon expression.
