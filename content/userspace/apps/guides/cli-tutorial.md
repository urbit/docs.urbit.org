...
This tutorial can be considered to be an application equivalent of the [Hoon school lesson](urbit-docs/courses/hoon-school/P-stdlib-io#ask-generators) on `sole` and `%ask` generators, which only covers the bare minimum necessary to write generators that take user input.
