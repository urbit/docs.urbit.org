...
That's because our `desk.docket-0` file includes a [`%glob-ames`](../reference/dist/docket#glob-ames) clause which specifies our ship as the source, so it's waiting for us to upload the glob of front-end files.
...
We can now create a glob from the `hello-glob` directory we previously created. To do so, navigate to `http://localhost:8080/docket/upload` in the browser. This will bring up the `%docket` app's [Globulator](../reference/dist/glob#globulator) tool:
