...
In addition to the `$glob` type, a glob can also be output to Unix with a `.glob` file extension for distribution over HTTP. This file simply contains a [`jam`](urbit-docs/language/hoon/reference/stdlib/2p#jam)med `$glob` structure.
...
If an app binds an Eyre endpoint and handles HTTP directly, for example with a [`%connect` task:eyre](urbit-docs/system/kernel/eyre/reference/tasks#connect), the `%site` clause is used, specifying the Eyre binding. In this case a glob is omitted entirely.
