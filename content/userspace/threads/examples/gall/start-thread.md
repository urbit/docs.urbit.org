...
We pass the whole `$shed:khan` to Khan in a [`%lard`](../../../../system/kernel/khan/reference/tasks.md#lard) task card:
...
Khan will handle creating a thread ID, poking Spider to start it, and waiting for the result. Once done, Khan will return an [`%arow`](../../../../system/kernel/khan/reference/tasks.md#arow) gift back to our agent in `+on-arvo`, where we check if it succeeded and print the result:
...
In `+on-poke` we simply take `.ted` (the thread to run) and `.txt` (the starting argument string) and pass them to Kahn in a [`%fard`](../../../../system/kernel/khan/reference/tasks.md#fard) task card:
...
Then we create the [`$inline-args`](../../reference/api.md#start-thread) starting arguments. If you were running a thread file rather than an inline thread you'd use `$start-args` instead, which is very similarly structured, but specifies a thread filename and argument in a vase instead of the `$shed:khan`.
