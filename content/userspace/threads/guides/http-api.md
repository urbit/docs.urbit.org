...
Spider has an Eyre binding which allows threads to be run externally via [authenticated](urbit-docs/system/kernel/eyre/reference/external-api-ref#authentication) HTTP POST requests.
...
First we must obtain a session cookie by [authenticating](urbit-docs/system/kernel/eyre/guides/guide#authenticating).
